-- =============================================================================
-- CARTEIRA DO POP — usar a SAFRA MAIS RECENTE da tabela de índices.
-- Banco alvo: Supabase EXTERNO kmedldlepwiityjsdahz.
--
-- BUG LATENTE que esta migration fecha (encontrado em 15/08/2026, antes de
-- causar dano): `jm_indices` tem chave única (indice, competencia, REFERENCIA)
-- — ela guarda uma safra por data de referência, e não sobrescreve a anterior.
-- A versão de 20260815200000 fazia:
--
--     left join public.jm_indices idx
--            on idx.indice = ... and idx.competencia = ...
--
-- sem filtrar a referência. Hoje existe UMA safra só por índice (2026-07-01),
-- então o join casa uma linha e tudo funciona. Na primeira safra nova — que o
-- sync com o Bacen vai criar todo mês — o mesmo (indice, competencia) passaria a
-- casar DUAS linhas, o join multiplicaria as partes do processo e a carteira
-- inteira duplicaria. Silenciosamente.
--
-- A correção: `distinct on (indice, competencia) ... order by referencia desc`
-- elege a safra vigente antes do join. O histórico continua na tabela (dá para
-- responder "quanto a carteira valia corrigida até jul/2026"), mas a carteira
-- sempre lê a mais recente.
--
-- Nada mais muda em relação a 20260815200000.
--
-- REVERSÃO: re-executar 20260815200000_pop_carteira_marcos_valor_atualizado.sql
-- — MAS só faça isso enquanto houver uma única safra por índice, senão a
-- carteira duplica. Se já houver mais de uma, a reversão correta é apagar as
-- safras extras junto.
-- =============================================================================
