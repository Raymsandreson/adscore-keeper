-- Fase 2: libera fusão de casos e processos na whitelist da RPC.
-- A mecânica é a mesma (grafo de FK em runtime): legal_cases é referenciado por
-- lead_financials/zapsign_documents/lead_activities/lead_processes/etc; lead_processes
-- por lead_activities/process_movements/process_parties/etc — tudo re-vinculado sozinho.
-- Detecção segura no front: processo por nº CNJ, caso por mesmo cliente (lead_id),
-- nunca pelo rótulo "PREV nnn". Fusão de caso não é auto-selecionada (mexe em honorário).
...
begin
  if p_table not in ('leads', 'contacts', 'legal_cases', 'lead_processes') then
  ...
  -- OBS: EXECUTE ... INTO NAO altera FOUND; por isso capturamos um "true" da propria linha.
