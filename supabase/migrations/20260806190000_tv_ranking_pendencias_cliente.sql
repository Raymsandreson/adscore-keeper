-- Telão: critério novo — PENDÊNCIAS DO CLIENTE em aberto, como desempate logo
-- depois de ATRASADAS.
--
-- Pedido do Raym em 06/08/2026: o que o cliente ficou de fazer e ninguém
-- resolveu também precisa pesar no ranking, logo abaixo das atividades
-- atrasadas.
--
-- Semântica:
--   pend_cliente = pendências de `lead_client_commitments` em aberto
--                  (status 'combinado' ou 'cobrado'), atribuídas a quem cuida
--                  do caso. Menos é melhor (asc), igual a 'atrasadas'.
--   Backlog TOTAL, sem filtro de período — dívida velha não some quando vira
--   o dia, mesma regra de 'atrasadas' e 'fb_pendentes'.
--
-- A quem a pendência é atribuída (cascata, o primeiro que existir):
--   1. responsável do processo mais recente do lead (lead_processes.responsible_user_id)
--   2. responsável processual do lead (leads.processual_responsible_id)
--   3. último assessor que trabalhou o lead (lead_activities.assigned_to)
--
-- LIMITE MEDIDO EM 06/08/2026, antes de aplicar: das 179 pendências abertas,
-- só 40 (22%) chegam a ter dono por essa cascata — 7 pelo responsável
-- processual, o resto pelo assessor da atividade. Pendência sem dono NÃO
-- pontua contra ninguém (fica de fora do count), então o critério é
-- conservador por construção: ele só desempata onde o caso tem responsável
-- definido. Conforme o cadastro de responsável melhorar, o critério passa a
-- medir mais — sem precisar mexer aqui.
--
-- Ordem do ranking depois desta migration:
--   status esperado > fases > objetivos > passos > itens > concluídas >
--   MENOS ATRASADAS > MENOS PENDÊNCIAS DO CLIENTE > maior média ⭐ >
--   menos feedbacks sem avaliar > mais tempo ativo > menos ocioso > chat
--
-- Também entra o ramo 'pend_cliente' em tv_ranking_detalhe, para o chip abrir a
-- lista itemizada (cliente, o que ficou de fazer, há quantos dias, cobranças).
--
-- NÃO muda: quem entra no ranking (`ranked` continua exigindo entrega no
-- período — ninguém aparece só por ter pendência), as demais CTEs, nem a versão
-- legada de 2 args.
--
-- Rollback: reaplicar 20260805120000_tv_ranking_estrelas_e_feedback_pendente.sql
-- (as duas funções voltam sem o campo; o front lê com fallback 0 e a coluna
-- some sem quebrar).
...
itens as (
  ...
  select -extract(epoch from c.promised_at) as ord,
    jsonb_build_object(
      ...
      'titulo', c.title,
      'lead_id', c.lead_id,
      'lead_nome', coalesce(l.lead_name, c.phone),
  ...
  where p_criterio = 'pend_cliente'
    ...
    and coalesce(
      ...
      (select a.assigned_to from lead_activities a
        where a.lead_id = c.lead_id and a.deleted_at is null and a.assigned_to is not null
        order by a.created_at desc limit 1)
    ) in (select ext_user from ext_users)
