-- Telão: PENDÊNCIAS DO CLIENTE viram dois números no card — FEITAS e FALTANDO —
-- logo depois de ATRASADAS.
--
-- Pedido do Raym em 07/08/2026: "colocar no ranking também as pendências, o nº
-- de pendências feitas e faltantes, depois de atv atrasadas". Até aqui só o
-- backlog em aberto (`pend_cliente`) existia, e o card do Modo Corrida nem
-- mostrava esse número.
--
-- O que entra:
--   pend_feitas  = pendências marcadas como cumpridas (status 'feito') com
--                  done_at DENTRO do período do telão (hoje/semana/mês, mesmo
--                  p_since das concluídas). Mais é melhor (desc).
--   pend_cliente = backlog em aberto ('combinado'/'cobrado'), sem filtro de
--                  período, como já era. Menos é melhor (asc).
--
-- Ordem depois desta migration:
--   status esperado > fases > objetivos > qualidade (>=3 notas) > passos >
--   itens do checklist > concluídas > menos atrasadas > MAIS PENDÊNCIAS FEITAS >
--   menos pendências faltando > maior média ⭐ > menos feedbacks sem avaliar >
--   mais tempo ativo > menos ocioso > resposta no chat > nome.
--
-- MUDANÇA DE DONO (importante): a cascata inline de 3 degraus sai e as duas
-- contagens passam a ler `vw_client_commitments_owner.owner_user_id` — a MESMA
-- fonte da caixa de pendências, que desde 20260807170000 tem `assigned_to`
-- (troca manual) como degrau 0 e ainda resolve por dono da conversa e dono da
-- instância. Sem isso, trocar o responsável de uma pendência na caixa não
-- mexia no telão, e "feitas" e "faltando" poderiam cair em pessoas diferentes.
--
-- Efeito medido no Externo em 07/08/2026, antes de aplicar:
--   441 pendências abertas — 87 tinham dono pela cascata antiga, 162 têm pela
--   view. Ou seja: o número de "faltando" sobe para quem já era dono do caso
--   por conversa/instância. Pendência sem dono continua não pontuando contra
--   ninguém.
--   325 feitas no total (todas de 06 e 07/08, quando a feature nasceu); 315
--   foram marcadas pela IA ao ver a conversa — por isso o crédito é do DONO do
--   caso, não de `done_by` (que é quase sempre nulo/IA e zeraria o critério).
--
-- Também entra o ramo 'pend_feitas' em tv_ranking_detalhe (o chip abre a lista
-- do que o cliente cumpriu no período) e o ramo 'pend_cliente' passa a usar a
-- mesma view, pra lista e contagem não divergirem.
--
-- NÃO muda: quem entra no ranking (`ranked` continua exigindo entrega no
-- período), qualquer outro critério, as demais CTEs, nem a versão legada de 2
-- args.
--
-- ROLLBACK (verificado): reaplicar, sem alteração, os dois arquivos anteriores
--   supabase/migrations/20260806230000_tv_ranking_qualidade_com_piso.sql  (ranking)
--   supabase/migrations/20260806190000_tv_ranking_pendencias_cliente.sql  (detalhe)
-- Ambos são create or replace da função inteira; o front lê `pend_feitas` com
-- fallback 0 e o número some sem quebrar a tela.
...
-- Pendências do cliente, pelas duas pontas: o que ele CUMPRIU no período e o
-- que continua FALTANDO (backlog total). Dono = owner_user_id da view, a mesma
-- da caixa de pendências (assigned_to > processo > responsável processual >
-- último assessor > dono da conversa > dono da instância). Pendência sem dono
-- não conta nem a favor nem contra ninguém.
pend_cli as (
  select c.owner_user_id as ext_user,
    count(*) filter (where c.status = 'feito' and c.done_at >= p_since)::int as pend_feitas,
    count(*) filter (where c.status in ('combinado','cobrado'))::int as pend_cliente
  from vw_client_commitments_owner c
...
merged as (
  select
    ...
    coalesce(pc.pend_feitas, 0) as pend_feitas,
...
named as (
  select
    ...
    f.estrelas_soma, f.notas_n, f.fb_pendentes, f.pend_feitas, f.pend_cliente,
...
by_name as (
  select
    ...
    sum(pend_feitas)::int as pend_feitas,
  ...
  'ranking', (
    select coalesce(
      jsonb_agg(row_to_json(r) order by
        ...
        -- pendências do cliente, nas duas pontas: mais cumpridas no período
        -- primeiro, depois menos faltando no backlog.
        r.pend_feitas desc, r.pend_cliente asc,
...
-- tv_ranking_detalhe: base = 20260806190000 (pendências do cliente), com duas
-- diferenças e nada mais:
--   1. ramo novo 'pend_feitas' (o que o cliente cumpriu no período);
--   2. ramo 'pend_cliente' passa a resolver o dono por
--      vw_client_commitments_owner, igual ao ranking — antes era a cascata
--      inline de 3 degraus e a lista podia divergir da contagem do chip.
...
itens as (
  ...
  -- PENDÊNCIAS DO CLIENTE em aberto atribuídas a quem cuida do caso. Backlog
  -- total, igual a 'atrasadas' e 'fb_pendentes' — dívida velha não some quando
  -- vira o dia. Mais antiga primeiro (ord negativo).
  -- 07/08/2026: o dono sai de vw_client_commitments_owner (mesma fonte da caixa
  -- de pendências e do ranking), em vez da cascata inline de 3 degraus.
  select -extract(epoch from c.promised_at) as ord,
    jsonb_build_object(
      'tipo', 'pend_cliente',
      'quando', c.promised_at,
      'titulo', c.title,
      'lead_id', c.lead_id,
      'lead_nome', coalesce(c.lead_name, c.phone),
      'dias_parado', (current_date - (c.promised_at at time zone 'America/Sao_Paulo')::date),
      'prazo', c.due_date,
      'cobrancas', c.reminder_count,
      'origem', c.origin,
      'trecho', left(btrim(coalesce(c.source_message_text, '')), 300)
    )
  from vw_client_commitments_owner c
  where p_criterio = 'pend_cliente'
    and c.status in ('combinado','cobrado')
    and c.owner_user_id in (select ext_user from ext_users)

  union all

  -- PENDÊNCIAS QUE O CLIENTE CUMPRIU no período do telão (status 'feito' com
  -- done_at >= p_since). Mais recente primeiro. Quem marcou costuma ser a IA ao
  -- ver a conversa — o crédito é do dono do caso, igual à contagem do ranking.
  select extract(epoch from c.done_at) as ord,
    jsonb_build_object(
      'tipo', 'pend_feita',
      'quando', c.done_at,
      'titulo', c.title,
      ...
      'cobrancas', c.reminder_count,
      'origem', c.origin,
      'quem_marcou', nullif(btrim(coalesce(c.done_by_name, '')), ''),
      'dias_ate_cumprir', greatest(
        (c.done_at at time zone 'America/Sao_Paulo')::date
          - (c.promised_at at time zone 'America/Sao_Paulo')::date, 0),
      'trecho', left(btrim(coalesce(c.source_message_text, '')), 300)
    )
  from vw_client_commitments_owner c
  where p_criterio = 'pend_feitas'
    and c.status = 'feito'
    and c.done_at >= p_since
    and c.owner_user_id in (select ext_user from ext_users)
