-- Classificação HOME OFFICE × ESCRITÓRIO dos membros no ranking do telão.
--
-- 1) org_user_status ganha a coluna home_office — marcada por membro na aba
--    Times (toggle 🏠/🏢). Chave segue sendo o Cloud auth user_id.
-- 2) tv_atividades_ranking devolve home_office em cada linha do ranking — o
--    telão /tv/atividades exibe o selo 🏠 e compara a produção média
--    (passos/pessoa) de quem está no escritório vs fora.
--
-- Única mudança vs a versão viva (20260721191000_tv_ranking_ignora_passo_
-- retroativo): CTE `regime` + join em `named` + bool_or em `by_name`.
-- Assinatura inalterada (timestamptz, uuid, text) → create or replace.
-- Rollback: re-rodar 20260721191000_tv_ranking_ignora_passo_retroativo.sql
-- (a coluna home_office pode ficar — passa a ser ignorada).

alter table public.org_user_status
...
chat as (
  ...
  from (
    ...
    union all
    -- GRUPO: só menção; tempo da mensagem que mencionou até a primeira
    -- resposta da pessoa mencionada naquela conversa.
