-- Filtro "Gerencial e Diretoria" no telão /tv/atividades.
--
-- Nova opção no seletor de time: mostra só quem é gestor de time
-- (team_managers.manager_user_id) ou diretor (org_directors.user_id).
-- Ambas as tabelas guardam UUID do auth do CLOUD → mapeia pra conta do
-- Externo via auth_uuid_mapping, como as CTEs `passos` e `chat`.
--
-- Assinatura ganha p_grupo text ('gerencial' | null). A versão de 2 args é
-- DROPADA — se ficasse, a chamada antiga do front (2 args nomeados) viraria
-- "function is not unique" no PostgREST. O front antigo continua funcionando
-- na nova versão porque p_grupo tem default null.
--
-- No modo gerencial o nome exibido vem de team_managers/org_directors, não do
-- profile: perfil de diretor pode ser username sem sobrenome (ex.:
-- "raymsandresonadv"), que o filtro anti-conta-de-sistema do by_name
-- descartaria.
--
-- Rollback: drop function tv_atividades_ranking(timestamptz, uuid, text);
-- e re-rodar 20260721150000_tv_ranking_chat_resposta.sql.

drop function if exists public.tv_atividades_ranking(timestamptz, uuid);
