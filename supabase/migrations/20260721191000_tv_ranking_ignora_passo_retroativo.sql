-- Ranking do telão ignora passo RETROATIVO (marcado hoje, mas que o assessor
-- declarou já ter acontecido antes — flag metadata.retroactive gravado pela
-- log_checklist_step 4-arg, migration irmã 20260721190000).
--
-- Única mudança vs a versão viva (20260721180000_chat_resposta_so_privado_ou_
-- mencao): o filtro de retroativo na CTE `passos`. Logs antigos não têm a
-- chave no metadata → coalesce trata como não-retroativo (contam igual antes).
--
-- Assinatura inalterada (timestamptz, uuid, text) → create or replace.
-- Rollback: re-rodar 20260721180000_chat_resposta_so_privado_ou_mencao.sql.
