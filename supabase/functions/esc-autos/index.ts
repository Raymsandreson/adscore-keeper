...
function normalizaItem(d, origem) {
...
Deno.serve(async (req)=>{
  ...
  if (url.searchParams.get("k") !== GUARD) return new Response("forbidden", {
    status: 403
  });
  ...
  if (acao === "arquivar") {
    ...
    const amostra = [];
    const processarUm = async (d)=>{
      processados++;
      try {
        const r = await fetch(d.link_api, {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        const { error: updErr } = await sb.from("jm_documentos").update({
          storage_path: spath,
        ...
        sucesso++;
        if (amostra.length < 3) amostra.push({
          id: d.id,
          bytes: buf.length,
          ct
        });
    ...
    while(Date.now() - inicio < budgetMs){
      let q = sb.from("jm_documentos").select("id, processo_cnj, link_api").is("storage_path", null).is("storage_error", null).not("link_api", "is", null);
    ...
    const { count: restantes } = await sb.from("jm_documentos").select("id", {
      count: "exact",
      head: true
  ...
  if (acao === "autos" || acao === "docs") {
    ...
    if (acao === "autos") {
      const st = await fetch(`${HOST}/api/v2/processos/numero_cnj/${cnj}/status-atualizacao`, {
        headers: h
      });
      if (st.ok) {
        ...
        if (ver?.status === "PENDENTE") {
          return json({
            ...
            acao,
            aguardando: true,
            motivo: "ATUALIZACAO_EM_ANDAMENTO"
          });
        }
        // A atualizacao com certificado JA falhou no tribunal, e o motivo real
        // (SECRET_ERROR = TOTP recusado, LOGIN_ERROR = login recusado) so existe
        // aqui. Antes disto a funcao seguia, tomava um 422 generico no /autos, o
        // rebaixamento recolhia os publicos e a conclusao gravava
        // motivo_erro = null: sumia o unico rastro de uma consulta de R$ 1,50.
        // Foram 13 assim em 21/08/2026, todas registradas como SUCESSO.
        if (ver?.status === "ERRO" && ver?.motivo_erro) {
          await sb.from("jm_esc_solicitacoes").update({
            status: "A_ENVIAR",
            modo: "PUBLICOS",
            autos_erro: ver.motivo_erro,
            autos_tentado_em: new Date().toISOString(),
            motivo_erro: `autos recusados pelo tribunal: ${ver.motivo_erro}`
    ...
    const itens = [];
    let proxima = `${HOST}/api/v2/processos/numero_cnj/${cnj}/${endpoint}?limit=100`;
    let paginas = 0;
    while(proxima && paginas < 30){
      ...
      if (!r.ok) {
        ...
        // Sem permissão nos autos não é falha permanente: a solicitação volta
        // para a fila em modo PUBLICOS em vez de morrer.
        const rebaixa = acao === "autos" && r.status === 422;
        await sb.from("jm_esc_solicitacoes").update(rebaixa ? {
          status: "A_ENVIAR",
          modo: "PUBLICOS",
          motivo_erro: erro,
          autos_erro: `HTTP_422 ${erro.slice(0, 120)}`,
          autos_tentado_em: new Date().toISOString()
        } : {
          status: "ERRO",
          motivo_erro: erro
    ...
    const { data: gravado, error: rpcErr } = await sb.rpc("jm_documentos_ingerir", {
      p_cnj: cnj,
      p_itens: itens
    });
    ...
    // `autos_erro` so e limpo por uma colheita de autos que funcionou. A
    // colheita publica NAO apaga o historico da tentativa cara que falhou.
    await sb.from("jm_esc_solicitacoes").update({
      status: "SUCESSO",
  ...
  let r;
  if (acao === "get" && typeof path === "string" && path.startsWith("/api/")) r = await fetch(`${HOST}${path}`, {
    headers: h
  });
  else if (acao === "solicitar") r = await fetch(`${HOST}/api/v2/processos/numero_cnj/${cnj}/solicitar-atualizacao`, {
    method: "POST",
    headers: h,
    body: JSON.stringify(rawBody ?? {})
  });
  else if (acao === "status") r = await fetch(`${HOST}/api/v2/processos/numero_cnj/${cnj}/status-atualizacao`, {
    headers: h
  });
  else return json({
    ok: false,
    cnj: cnj ?? null,
    motivo: "ACAO_INVALIDA"
  });
  ...
  let resposta = t.slice(0, 2000);
  try {
    resposta = JSON.parse(t);
  } catch  {}
  return json({
    ok: r.ok,
    cnj: cnj ?? null,
    http_status: r.status,
    creditos: r.headers.get("Creditos-Utilizados"),
    resposta
  });
});
