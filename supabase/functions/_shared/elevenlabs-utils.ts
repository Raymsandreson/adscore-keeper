...
/**
 * Check ElevenLabs subscription credits via GET /v1/user/subscription
 */
export async function checkElevenLabsCredits(apiKey: string): Promise<ElevenLabsSubscription> {
  ...
  if (!res.ok) {
    ...
    // Return optimistic result so we don't block on subscription check failures
...
/**
 * Retry a fetch call up to `maxRetries` times on transient failures (429, 5xx).
 */
export async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (!isRetryable || attempt === maxRetries) {
      return res; // Return failed response for caller to handle
    }
