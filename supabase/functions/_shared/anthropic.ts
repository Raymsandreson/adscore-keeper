/**
 * Shared Anthropic (Claude) AI helper for all edge functions.
 ...
 * so callers (and gemini.ts's dispatcher) don't need to change.
...
/** Convert one OpenAI-compatible content part into an Anthropic content block. */
function convertContentPart(p: any): any {
  ...
  if (p.type === "input_audio") {
    // Anthropic Messages API does not accept audio input. Audio agents must
    // stay on the Gemini path (keep their model as google/*).
...
/**
 * Call the Anthropic Messages API with OpenAI-compatible input format.
 * Returns the raw fetch Response (body is Anthropic JSON or SSE if stream).
 */
export async function callAnthropic(options: AnthropicCallOptions): Promise<Response> {
  ...
  return fetch("https://api.anthropic.com/v1/messages", fetchOptions);
}

/** Parse a non-streaming Anthropic response into OpenAI-compatible format. */
...
/**
 * High-level helper: call Anthropic and return parsed OpenAI-compatible result.
