...
//      respeitando cooldown por motivo (3/7/30 dias) e teto por rodada.
//      Cada solicitação grava linha em radar_atualizacoes com os créditos que
//      o Escavador cobrou (header Creditos-Utilizados) — custo auditável.
//
...
Deno.serve(async (req) => {
  try {
    ...
    // Cooldown: última solicitação (qualquer status ≠ ERRO) por CNJ em 30 dias.
    const cnjsParados = parados.map((q) => q.processo_cnj);
    ...
    const elegiveis = parados.filter((q) => {
    ...
    resumo.solicitacoes = {
      parados: parados.length, elegiveis: elegiveis.length,
