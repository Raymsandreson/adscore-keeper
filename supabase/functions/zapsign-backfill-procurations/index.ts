...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Modo probe_phone: testa match em CLOUD vs EXTERNAL
    if (probePhone) {
      const last8 = probePhone.replace(/\D/g, "").slice(-8);
      ...
      if (extUrl && extKey) {
        ...
        // sample columns
        const { data: extOne, error: extColErr } = await ext.from("leads").select("*").limit(1);
        out.external_columns = extOne && extOne[0] ? Object.keys(extOne[0]) : [];
        out.external_col_error = extColErr?.message || null;
      } else {
