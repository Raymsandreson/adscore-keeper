  ...
  try {
    ...
    if (!zapsignToken) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "ZAPSIGN_API_TOKEN not configured",
        }),
        {
          status: 500,
    ...
    if (action === "get_template") {
      ...
      if (!template_token) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "template_token is required",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const response = await fetch(
        `${ZAPSIGN_API_URL}/templates/${template_token}/`,
        {
          headers: zapsignHeaders,
        },
      );
    ...
    if (action === "create_doc") {
      ...
      if (!template_id || !signer_name) {
        return new Response(
          JSON.stringify({
            success: false,
            error: "template_id and signer_name are required",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      const apiPhoneCountry = cleanPhoneApi.startsWith("55")
        ? "55"
        : cleanPhoneApi.substring(0, 2);
      const apiPhoneNumber = cleanPhoneApi.startsWith("55")
        ? cleanPhoneApi.substring(2)
        : cleanPhoneApi;
      const filledTemplateData = (templateData && Array.isArray(templateData))
        ? templateData.filter((item: any) =>
          item?.de && item?.para && String(item.para).trim() !== ""
        )
        : [];
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `ZapSign create doc error: ${response.status} - ${errorText}`,
        );
      }
      ...
      if (Array.isArray(additionalSigners) && additionalSigners.length > 1) {
        for (let i = 1; i < additionalSigners.length; i++) {
          ...
          try {
            const addSignerRes = await fetch(
              `${ZAPSIGN_API_URL}/docs/${docData.token}/add-signer/`,
              {
                method: "POST",
                headers: zapsignHeaders,
                body: JSON.stringify({
            ...
            if (addSignerRes.ok) {
              ...
              console.log(
                `Added extra signer ${extraSigner.name}:`,
                addedSigner.token,
              );
              ...
              console.error(
                `Failed to add signer ${extraSigner.name}:`,
                errText,
              );
            }
          } catch (signerErr) {
            console.error(
              `Error adding signer ${extraSigner.name}:`,
              signerErr,
            );
          }
        }
      ...
      const allSignUrls = (docData.signers || []).map((
        ...
        name: s.name || additionalSigners?.[idx]?.name ||
          `Signatário ${idx + 1}`,
    ...
    if (action === "get_doc_status") {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `ZapSign get doc error: ${response.status} - ${errorText}`,
        );
      }
...
`;
      ...
      const prompt =
        `Você é um assistente jurídico especializado em extrair dados para preencher documentos. Analise TODAS as fontes disponíveis: dados do CRM, conversa do WhatsApp e IMAGENS de documentos.
...
Responda APENAS o JSON, sem markdown.`;
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "Você é um assistente especializado em extrair dados de conversas, imagens e documentos para preencher documentos jurídicos. Analise cuidadosamente TODAS as imagens enviadas, incluindo documentos de identidade, comprovantes e certidões. Extraia nacionalidade, estado civil, endereço e todos os dados visíveis. Responda apenas JSON válido.",
            },
        ...
        if (Array.isArray(extractedData)) {
          for (const field of extractedData) {
            const fieldName = (field.de || "").replace(/\{\{|\}\}/g, "")
              .toUpperCase().trim();
            // Set default EMAIL and WHATSAPP for procuração documents
            if (
              (fieldName === "EMAIL" || fieldName.includes("EMAIL")) &&
              !field.para
            ) {
              field.para = "contato@prudencioadv.com";
            }
            if (
              (fieldName === "WHATSAPP" || fieldName.includes("WHATSAPP")) &&
              !field.para
            ) {
    ...
    if (action === "extract_signers") {
      ...
      const signerPrompt =
        `Você é um assistente jurídico. Analise a conversa do WhatsApp, imagens de documentos e dados do CRM para identificar QUEM vai assinar o documento e se há TESTEMUNHAS mencionadas.
...
Responda APENAS o JSON, sem markdown.`;
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "Você extrai informações de signatários e testemunhas de conversas e documentos. Responda apenas JSON válido.",
            },
            { role: "user", content: userContent },
          ],
        });
        ...
        try {
          extractedSigners = JSON.parse(
            respText.replace(/```json\n?/g, "").replace(/```\n?/g, "").trim(),
          );
        } catch {
        ...
        return new Response(
          JSON.stringify({
            success: true,
            signers: Array.isArray(extractedSigners) ? extractedSigners : [],
          }),
          { headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      } catch (err) {
