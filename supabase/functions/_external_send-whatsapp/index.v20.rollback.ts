// ROLLBACK BACKUP — send-whatsapp v20 (projeto externo kmedldlepwiityjsdahz)
// Salvo antes do deploy do fix de fallback-on-disconnect. Para reverter, redeploy deste conteúdo.
// @ts-ignore
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Retry com instâncias alternativas se a primeira não estiver no grupo
    if (!ur.ok) {
      const et0 = await readSafe(ur);
      const isGroup = isGroupJid(target) || target.replace(/\D/g, '').length > 15;
      if (isGroup && /not participating/i.test(et0)) {
        console.warn(`[send-whatsapp] ${inst.instance_name} not in group ${target}, trying fallbacks`);
        const phoneClean = target.replace(/@.*$/, '').trim();
      ...
      if (!ur.ok) {
        ...
        if (isDisc(ur.status, et)) return jsonResp(discPayload(usedInst.instance_name, et));
        return jsonResp({
          success: false,
