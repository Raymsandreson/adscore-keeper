...
export const handler: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (alreadyAnalyzed && !force) {
      ...
      return ok({ success: true, created: 0, commitments: [], cached: true });
    }
...
- Coisa que o cliente JÁ FEZ na própria conversa (mandou o documento e ele está lá) — se foi cumprido, não registre.
...
${existingTitles.size > 0
  ...
  : 'Nada registrado ainda nesta conversa.'}`;
              ...
              properties: {
                commitments: {
                  ...
                  items: {
                    ...
                    required: ['title', 'quote', 'source_message_id', 'due_date', 'confidence', 'kind'],
    ...
    console.log(`[detect-client-commitments] ok — msgs=${messages.length}, detectadas=${detected.length}, novas=${created}, duplicadas=${duplicates}`);

    return ok({ success: true, created, analyzed: messages.length, detected: detected.length });
  } catch (e: any) {
