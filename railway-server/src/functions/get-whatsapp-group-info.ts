...
export const handler: RequestHandler = async (req, res) => {
  try {
    ...
    // Monta lista de instâncias: a indicada primeiro, depois todas as ativas
    const tried: string[] = [];
    ...
    if (!foundName) {
      return res.json({ success: false, error: 'Nenhuma instância retornou o nome do grupo', tried });
    }

    // Persiste no banco se houver lead_id
    if (lead_id) {
      try {
        await ext
    ...
    return res.json({ success: true, name: foundName, used_instance: usedInstance });
  } catch (err: any) {
