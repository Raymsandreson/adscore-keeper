/**
 * whatsapp-cloud-templates — templates aprovados da WABA ativa, para a tela.
 *
 ...
 * WABA. Esta aqui só LÊ, e nem aceita `waba_id` do chamador — resolve pela
 * config ativa, então não dá pra sondar WABA de terceiro.
 *
...
export const handler: RequestHandler = async (_req, res) => {
  ...
  try {
    ...
    if (!wabaId) {
      res.status(200).json({ success: false, error: 'Cloud API sem WABA ativa configurada', templates: [] });
      return;
    }
    ...
    res.status(200).json({ success: true, waba_id: wabaId, templates });
  } catch (err) {
    res.status(200).json({
      success: false,
