...
| **autoria-do-envio-whatsapp** | Sempre que a conversa envolver "quem mandou" uma mensagem, identificar remetente, `external_message_id`, ou ao mexer em qualquer bolha de conversa. | Como o sistema sabe quem da equipe enviou cada mensagem: tabela `whatsapp_message_authors`, edge `send-whatsapp` v28 (autor vem do JWT, nunca do body), assinatura `*Nome:*` como fallback, e a regra de que a chave de casamento é o id da mensagem SEM o prefixo do dono. |
...
| **deploy-edge-function-verificado** | Sempre que for subir qualquer edge function, ou na dúvida sobre qual versão está no ar. | Autorização durável para deployar sem perguntar (set/2026) + o protocolo que torna isso seguro: `SUPABASE_PAT` não existe no ambiente remoto (usar o MCP), conferir o que está REALMENTE no ar com `get_edge_function` porque o espelho de rollback do repo já mentiu, subir numa canary, comparar `ezbr_sha256` e fazer smoke test por `pg_net`. |
