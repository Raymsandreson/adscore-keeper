...
export default function AddProcessDialog({ open, onOpenChange, caseId, leadId, onProcessAdded, boards = [] }: AddProcessDialogProps) {
  ...
  const saveSelectedFromEscavador = async () => {
    ...
    if (processType === 'judicial' && !responsibleExtId) {
    ...
    try {
      ...
      for (const idx of selectedResults) {
        ...
        if (error) {
          ...
          try {
            const extUserId = await remapToExternal(user?.id);
            const { extAssignedTo, assignedName } = await resolveAssignment(title, caseId, user?.id, processType, result.numero_cnj);
  ...
  const saveManual = async () => {
    ...
    try {
      ...
      // Auto-create/attach "Dar andamento" activity via helper central
      try {
        const { extAssignedTo, assignedName } = await resolveAssignment(manualForm.title.trim(), caseId, user?.id, processType, manualForm.process_number);
        const r = await createOrAttachAndamentoActivity({
          leadId,
          caseId,
          processId: insertedProcess?.id || null,
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-3 border rounded-lg p-3 bg-muted/30">
          ...
          <div>
            <Label className="text-xs font-semibold">
              Responsável pelo processo {processType === 'judicial' && '*'}
            </Label>
            ...
            />
            {processType === 'judicial' && !responsibleExtId && (
        ...
        <Tabs value={tab} onValueChange={v => setTab(v as any)}>
          ...
          <TabsContent value="escavador" className="space-y-3 mt-3">
            ...
            {selectedResults.size > 0 && (
              <Button
                ...
                disabled={saving || (processType === 'judicial' && !responsibleExtId)}
                className="w-full"
              >
          ...
          <TabsContent value="manual" className="space-y-3 mt-3">
            ...
            <Button onClick={saveManual} disabled={saving || (processType === 'judicial' && !responsibleExtId)} className="w-full">
