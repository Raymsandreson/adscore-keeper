...
import { Loader2, Send, Play, RotateCcw, User, Search, X, Pencil, Save, Download, CheckCheck, Paperclip, Mic, Square, Zap, Eye, ChevronDown, ChevronRight } from 'lucide-react';
...
export function AgentTestChat({ systemPrompt, model = 'google/gemini-2.5-flash', agentName, onPromptChange, proactiveEnabled, proactiveInstruction }: Props) {
  ...
  const loadRealConversation = async () => {
    ...
    try {
      const n = await seedMessagesFromPhone(phone);
      if (n === 0) toast.error('Nenhuma mensagem encontrada pra esse lead');
      else toast.success(`Carregadas ${n} mensagens reais`);
    } catch (e: any) {
      console.error(e);
