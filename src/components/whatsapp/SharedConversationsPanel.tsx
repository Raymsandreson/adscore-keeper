...
import { Share2, MessageCircle, ClipboardPlus, Loader2, Inbox, Send } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useSharedWithMe, type SharedConversation } from '@/hooks/useSharedWithMe';
...
export function SharedConversationsPanel({ open, onOpenChange }: Props) {
  const { items, sharedByMe, loading, reload } = useSharedWithMe();
  ...
  const createActivity = async (share: SharedConversation) => {
    if (!user) return;
  ...
  const removeShare = async (id: string) => {
    try {
      await supabase.from('whatsapp_conversation_shares').delete().eq('id', id);
  ...
  const renderShare = (share: SharedConversation, kind: 'with_me' | 'by_me') => {
    const isNew = kind === 'with_me' && !share.acknowledged_at;
    const otherUser = kind === 'with_me' ? share.shared_by : share.shared_with;
    const label = kind === 'with_me' ? 'de' : 'para';
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] text-muted-foreground">
              {label} <span className="font-medium">{getProfileName(otherUser)}</span>
              {' · '}
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md p-0 flex flex-col">
        ...
        <Tabs defaultValue="with_me" className="flex-1 flex flex-col min-h-0">
          <TabsList className="mx-3 mt-3 grid grid-cols-2">
            <TabsTrigger value="with_me" className="text-xs gap-1.5">
              <Inbox className="h-3.5 w-3.5" />
              Comigo
              {unreadCount > 0 && <Badge variant="destructive" className="ml-1 h-4 px-1 text-[9px]">{unreadCount}</Badge>}
            </TabsTrigger>
            <TabsTrigger value="by_me" className="text-xs gap-1.5">
              <Send className="h-3.5 w-3.5" />
              Eu compartilhei
              {sharedByMe.length > 0 && <Badge variant="secondary" className="ml-1 h-4 px-1 text-[9px]">{sharedByMe.length}</Badge>}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="with_me" className="flex-1 mt-0 min-h-0">
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 px-4 text-muted-foreground gap-2">
                  ...
                  <p className="text-sm text-center">
                    Ninguém compartilhou conversas com você ainda.
                  </p>
                </div>
              ) : (
                <div className="p-3 space-y-2">
                  {items.map(s => renderShare(s, 'with_me'))}
                </div>
              )}
            </ScrollArea>
          </TabsContent>

          <TabsContent value="by_me" className="flex-1 mt-0 min-h-0">
            <ScrollArea className="h-full">
              {loading ? (
                <div className="flex items-center justify-center py-12">
                  <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
                ...
                <div className="flex flex-col items-center justify-center py-12 px-4 text-muted-foreground gap-2">
                  ...
                  <p className="text-sm text-center">
                    Você ainda não compartilhou nenhuma conversa.
                  </p>
                </div>
              ) : (
                <div className="p-3 space-y-2">
                  {sharedByMe.map(s => renderShare(s, 'by_me'))}
                </div>
              )}
            </ScrollArea>
          </TabsContent>
