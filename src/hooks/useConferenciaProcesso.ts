...
export interface ValorJm {
  ...
  /** Percentual de honorário sucumbencial da decisão (fração: 0.15 = 15%). */
  hs_pct: number | null;
    ...
    try {
      ...
      const [dec, val, pag, ppm, pm, dup] = await Promise.all([
        ...
        externo.from('jm_valores')
          .select('id, dec_id, cliente, dano_moral, dano_estetico, base_calculo, flag_correcao, hs_pct, dano_moral_anterior, dano_estetico_anterior, corrigido_em, corrigido_por_leitura')
          .in('processo_cnj', variantes) as unknown as Promise<Consulta>,
