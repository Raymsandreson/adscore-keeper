...
| Concluída — **indeferido** ou arquivado por prazo | Mensagem ao cliente | O veredito vem de `inss_admin_processes.resultado` ou do despacho |
| Concluída — **deferido** | Silêncio ao cliente, **atividade para Luana e José** | 04/09/2026: aprovação é conversa de gente. Ver "Deferimento não sai sozinho", abaixo |
...
- **O texto de reserva não passa pelo prompt** — e por isso repete as proibições dele na unha (`simplificarPendenciaParaReserva`, `lib/inss-despacho.ts`, 02/09/2026). Depois que a procuração voltou a ser pendência do cliente (01/09), o fallback passou a despejar o parágrafo jurídico inteiro no grupo: medido sobre as 400 exigências mais recentes, **28 das 64 de procuração** levavam `ZAPSIGN PROCESSAMENTO DE DADOS LTDA`, o endereço do validador do ITI, a MP 2.200-2 e a IN 128/2022 — dizer ao cliente que quem assinou foi a ZapSign, e não ele, lê como confissão do escritório. Agora o parágrafo de recusa vira uma frase (*"O INSS não aceitou a procuração do jeito que ela foi assinada. Precisa de uma assinada à mão, à caneta."*) e o fundamento legal sai, preservando o pedido acionável — em `1148944191`, "PROCURAÇÃO e o TERMO DE RESPONSABILIDADE" continua inteiro. **Só atua na exigência de procuração**: sem esse porteiro o corte levava junto a lista de quem é dispensado da biometria (24 casos) e um pedido de CTC do RPPS que cita a Portaria MTP 1.467/2022. Pós-fix nas mesmas 400: 0 termos proibidos, 0 prazos perdidos, 0 mensagens vazias nas 64; **0 alteradas nas 334 restantes**.
- Fila e auditoria em `inss_status_history`: `zap_status` (enviado|agendado|silencio|sem_grupo|repetido|retroativo|suprimido|expirado|suspeito|pericia_escritorio|so_escritorio|so_equipe|erro), `zap_tipo`, `zap_texto` (o texto exato que foi ao grupo), `zap_enviado_at`, `zap_erro`, e desde 04/09/2026 `zap_audio_status`/`zap_audio_url`. A coluna **não tem CHECK**, valor novo entra sem migration.

**O aviso vai falando — áudio junto da mensagem** (04/09/2026). Depois do texto sai uma nota de voz no mesmo grupo, pela mesma instância. `railway-server/src/lib/inss-audio-categoria.ts` decide **que** áudio; `inss-audio.ts` resolve e entrega; `enviarAudioAoGrupo` (`inss-zap.ts`) manda.

- **A regra que segura tudo é a unicidade.** O áudio gravado é genérico e fala de um assunto só, então ele só sai quando o despacho trata de **exatamente um** dos 13 assuntos reconhecidos. Despacho que pede procuração **e** biometria não recebe o áudio da procuração — receberia meia instrução e o cliente mandaria metade dos papéis. Nesse caso quem narra é o texto que foi enviado, que lista tudo. Mesma disciplina do match de procuração: nada de escolher por semelhança.
- **Três fontes, nesta ordem**: (1) o áudio **gravado** pela equipe, no catálogo `inss_audio_mensagens`; (2) o **roteiro fixo** da categoria, gerado por TTS **uma vez** e guardado no mesmo catálogo, para o assunto repetido não pagar geração nova; (3) o **texto da mensagem**, lido na hora, com cache por hash de (modelo, voz, texto) no storage.
- **Cobertura medida em 04/09/2026** sobre as 608 exigências com despacho, das quais **323 chegam ao cliente** (as outras 285 são de agendamento de perícia, que é tarefa do escritório): 27 procuração, 21 união estável, 16 biometria, 14 autodeclaração rural, 11 CTPS/CNIS, 4 pensão/óbito, 4 CAT, 4 documentos do CadÚnico, 4 outro benefício, 3 documento pessoal + residência, 3 laudo médico. As **212 restantes** misturam pedidos e são narradas pelo próprio texto.
- **Os áudios de CadÚnico têm pouco lastro no e-mail.** A equipe gravou três (atualizar no CRAS, documentos dos integrantes, estado civil), mas das 323 exigências que vão ao cliente, as 72 que citam CadÚnico o fazem como **rodapé jurídico** — "declaração no CadÚnico" na lista de comprovantes de residência aceitos, ou "dados do CadÚnico não podem ser utilizados na análise". Pedido de atualizar cadastro por exigência do INSS praticamente não existe nesta base; a cobrança de CadÚnico do BPC nasce por outro caminho.
- **Entidade HTML escapava dos regex**: parte dos despachos chega com `resid&ecirc;ncia`, `Identifica&ccedil;&atilde;o`. Sem `normalizarDespacho`, a lista genérica de documentos casava só em "Carteira de Trabalho" e virava, sozinha, categoria de CTPS — áudio errado por acidente de codificação.
- **Formato**: mp3 com `type: 'ptt'` na UazAPI. `ptt` é obrigatório (áudio comum chega no iPhone como "Este áudio não está mais disponível"), e mp3 é de propósito — a UazAPI reencoda o que não é ogg, e o reencode dela toca no iOS, enquanto ogg saído do nosso ffmpeg não toca em bitrate nenhum.
- **Áudio é acréscimo, nunca condição**: falha dele não desfaz o aviso, que já está no grupo. O que deu errado fica em `zap_audio_status` (`gravado` | `tts_catalogo` | `tts_texto` | `sem_audio` | `erro:<status>`).
- **A voz é a do José, clonada dos próprios áudios gravados** (04/09/2026, autorizado pelo usuário). `INSS_AUDIO_VOICE_ID=I47LzJiqO25W9lmdMwlr` na ElevenLabs, criada com as 10 gravações da equipe como amostra. Conferido por frequência fundamental: as gravações ficam em 88–113 Hz, a voz clonada em 96 Hz, e a voz de catálogo que servia de padrão até então (Laura, `FGY2WhTYpPnrIDTdsKH5`) em 205 Hz — feminina, num aviso onde todo o resto é masculino.
- **Trocar a voz não invalidava nada, e isso era invisível.** `chave` é UNIQUE no catálogo, então a linha gerada por TTS sobrevivia a qualquer troca de `INSS_AUDIO_VOICE_ID`: o cliente ouviria a voz antiga para sempre, sem erro e sem log. Foi o caso real do `protocolado`, o roteiro de maior volume (~200/mês), gerado em voz feminina de catálogo. Agora a linha guarda a coluna `voice_id`; divergindo da voz atual ela conta como ausente e é **regravada por cima** (upsert em `chave`, não insert — senão bateria em 23505). Linha `origem = 'gravado'` tem `voice_id` nulo e nunca é invalidada: é voz humana, não depende de modelo.
- **Ritmo em `speed 1.0`**, ajustável por `INSS_AUDIO_SPEED` (faixa 0.7–1.2 da API), e entra no hash do cache junto com a voz. Escolhido de ouvido comparando 1.0, 0.9 e 0.8 no mesmo texto e na mesma voz. O resto do projeto narra a **1.05–1.1** (telão, agente de vendas, command processor) — ritmo que não serve para explicar a um cliente idoso, ouvindo uma vez só, o que ele tem que providenciar. Os outros seis lugares ficaram como estão de propósito.
- **`inss-audio-preview` para ouvir antes do cliente ouvir** (`POST /functions/inss-audio-preview`, sob o gate de autenticação como toda função do Railway). Aceita `texto` ou uma `chave` de roteiro, `voice_id` e uma lista de `speed`, e gera pelo **mesmo `gerarTts`** do envio real. Existe porque até aqui uma mudança de voz só era audível depois de já ter ido a um grupo de cliente, e a única amostra disponível vinha do caminho de preview da edge `elevenlabs-voice-clone`, que roda a `speed: 1.1` — mais rápido do que a produção jamais gerou. Julgar locução por uma amostra gerada com outros parâmetros leva à decisão errada.
- **Desligar sem redeploy**: `INSS_AUDIO_ENVIAR=0` no Railway. Ligado por padrão. Teto de gerações novas: `INSS_AUDIO_MAX_HORA` (40). Voz: `INSS_AUDIO_VOICE_ID`, modelo `INSS_AUDIO_MODEL` (`eleven_multilingual_v2`).
- Testes: `src/lib/__tests__/inssAudioCategoria.test.ts` (12 casos com despacho real, incluindo os três falsos-positivos que a medição pegou: CadÚnico citado numa cobrança de alíquota, comunicado de vagas de perícia lido como Bolsa Família, e união estável lida como declaração de separação).

**Deferimento não sai sozinho** (04/09/2026, decisão do usuário). Aprovação deixou de virar mensagem automática: o que vem depois (conferir valores, combinar acesso ao Meu INSS, explicar data e local de pagamento) é conversa de gente, e quem fala precisa conhecer o caso. O evento continua classificado como `deferido` — é isso que permite seguir medindo quantas aprovações chegaram —, entra como `zap_status = 'so_equipe'` e vira **atividade para Luana e José**: a que já nascia ganha o roteiro, e sai uma irmã para quem faltou, cada uma dizendo que a outra pessoa recebeu a mesma tarefa. `mensagemVaiAoCliente` (`lib/inss-mensagem-cliente.ts`) é a porta única.

**A atividade sempre diz o que aconteceu com a mensagem** (04/09/2026, pedido do usuário). Já que a atividade é criada em todo evento — medido: **51 de 51** dos que falham têm uma —, ela é o lugar de explicar por que o cliente não recebeu. Até então só 6 dos 13 desfechos se explicavam: `silencio` (52 eventos), `retroativo` (26), `repetido` e `expirado` não diziam nada, e quem abria a atividade não tinha como saber se o cliente já sabia da atualização.

- **`explicarDestinoDaMensagem`** (`lib/inss-falha-envio.ts`) cobre todos os desfechos, inclusive o feliz — `✅ O CLIENTE JÁ FOI AVISADO` importa tanto quanto o resto, porque a pergunta que a pessoa faz ao abrir a atividade é "ele já sabe?". Devolve `null` só para os que já se explicam na descrição desde que ela nasce (vínculo suspeito, perícia do escritório, pendência só nossa) e para o deferimento, que tem roteiro próprio.
- **Cada desfecho pede uma atitude diferente**, e é isso que o texto entrega: `agendado` avisa que sai sozinha às 8h (para ninguém mandar em dobro à toa); `repetido` diz que o cliente já soube; `silencio` diz que não há o que contar; `retroativo` e `expirado` mandam conferir e avisar à mão.
- **Um bloco só, um `update` só.** Eram três `update` separados, cada um relendo a descrição original: dois avisos no mesmo evento se apagavam. Nunca foi observado em dado real — os 9 casos sem aviso são todos anteriores a 31/08, quando o aviso de `sem_grupo` nasceu — mas `sem_grupo` junto de procuração pendente já bastava para perder um dos dois.

**Envio recusado agora aparece na atividade** (04/09/2026, pedido do usuário). Sem grupo vinculado o robô já escrevia na atividade que o cliente não foi avisado; **envio recusado pela instância, não** — virava `zap_erro` numa coluna e um `console.log`, e nada reenviava. A assimetria era o defeito: a falha mais segura (não sei para onde mandar, então não mando) gritava, e a mais perigosa (tentei mandar e o WhatsApp recusou) era muda. Custo real: das duas falhas registradas até então, uma era um **indeferimento do próprio dia 04/09** — o cliente teve o benefício negado, o aviso não saiu porque a instância estava desconectada, e ninguém no escritório soube.

- **A causa é classificada, não só repassada** — `classificarFalha` (`lib/inss-falha-envio.ts`). Instância desconectada se resolve reconectando a instância; grupo inexistente se resolve revinculando o lead. Os dois chegam como 5xx da UazAPI (503 e 500), então o corte é pelo **texto** do erro, nunca pelo código HTTP. Aviso que não diz o que fazer vira ruído.
- **Indeferimento é destacado no aviso**, porque ali o prazo do cliente corre a partir de quando ele fica sabendo.
- **O José é avisado mesmo quando a atividade é de outra pessoa**: o aviso entra na atividade que já nasceu do evento e, se o dono não for ele, sai uma irmã — mesmo desenho do deferimento. Ele toca o pós-protocolo e é quem decide se avisa o cliente à mão.
- **A fila de 8h–20h também falhava calada** e não tem a atividade em mãos, porque roda horas depois num processo separado. O elo é o par **(lead, momento)**: o `notify-inss-update` cria a atividade ao processar o e-mail e só então agenda a mensagem, então a primeira atividade do robô naquele lead a partir de `email_received_at` é a deste evento. Não achando, o aviso vira atividade própria em vez de sumir.
- Testes: `src/lib/__tests__/inssFalhaEnvio.test.ts` (11 casos, os dois erros com o `zap_erro` literal do banco).

**Nome divergente também para de sumir calado** (04/09/2026). Quando o nome do segurado no requerimento não bate com nenhum nome do lead, o robô se recusa a mandar — a mensagem iria para o grupo de **outro cliente** (`zap_status = 'suspeito'`). A recusa está certa: medindo os 38 eventos parados assim, só **2** eram a mesma pessoa com grafia diferente (DANIELLE/DANIELE, RAINARA/RAYNARA) e **36 eram nomes de fato distintos**. **A atividade sempre avisou** — a descrição já trazia um bloco `⚠️ VÍNCULO SUSPEITO — a mensagem ao cliente foi BLOQUEADA`, presente em 35 das 38. O que faltava era peso e saída: o texto não distinguia decisão de andamento (são **17 indeferimentos e 7 deferimentos** entre 16/06 e 03/09) e não dizia que um vínculo errado prende TODAS as próximas atualizações.

- O aviso diz **o que não bateu** e cobre os dois desfechos: se o requerimento for deste lead, corrigir o nome no cadastro; se não for, achar o lead certo — senão **todas** as próximas atualizações daquele requerimento param no mesmo lugar.
- Junto com `erro`, os dois passaram a sair de um bloco só (`avisoDeNaoEntrega`), que já era o mesmo desenho. A ênfase por tipo cobre **deferimento** também: ali o cliente tem benefício aprovado e não sabe.
- **Nenhuma atualização se perde.** Medido em 04/09/2026 pelo número do requerimento, que está no título da atividade: **51 de 51** eventos em falha (`erro`, `suspeito`, `sem_grupo`) têm atividade. O que falha é a entrega ao cliente, nunca o registro interno. Cuidado ao medir: casar evento e atividade por janela de tempo dá falso negativo, porque a atividade nasce quando o `notify` roda e não quando o e-mail é sincronizado — um teste assim acusou 22 "perdas" que não existiam.
- **A régua para auditar este fluxo é contar `zap_status` por valor e perguntar, para cada um, quem fica sabendo e onde.** Se a resposta for "uma coluna" ou "um log", não fica sabendo ninguém — foi assim que o `erro` apareceu.
- **`sem_grupo` mede a fila de vínculo, não um defeito.** Dos 11 eventos, **11 tiveram o grupo vinculado depois** da falha (10 num lote em 03/09, um 14 minutos após o evento). O aviso na atividade está produzindo ação; o que não acontece é o reenvio do que ficou para trás.
- **O `suspeito` é dominado por BPC e maternidade, em que o segurado é a criança e o lead está em nome da mãe.** Dos 33 processos distintos, **14 compartilham sobrenome** com o nome do lead ou do grupo (MIGUEL FRANCA SEITH contra PALOMA … FRANÇA … SEITH; DANIELLE BUTZEN contra DANIELE BUTZEN). A causa de fundo é que a comparação usa `lead_name` e `group_name`, que são **rótulos** — carregam acolhedor, benefício, cidade e até manchete de notícia — enquanto `victim_name`, o campo que guardaria o beneficiário, está **nulo em todos** os casos examinados. Encher esse campo é o conserto estrutural; afrouxar a comparação de texto trocaria silêncio por mensagem no grupo errado.

**Indeferimento agora fala em ação judicial** (04/09/2026). O texto oferecia *"dá pra pedir pro INSS olhar de novo"* (recurso administrativo) enquanto o áudio gravado pela equipe promete *"a gente vai entrar com uma ação judicial buscando a concessão do seu benefício"* — o cliente receberia os dois no mesmo grupo, se contradizendo. Alinhado ao áudio, no fallback e na instrução da IA. Continua proibido prometer vitória.

**Salário-maternidade é a exceção: recurso, não ação** (08/09/2026, correção do usuário). O alinhamento de 04/09 valeu para **todo** benefício, e em salário-maternidade o caminho depois do indeferimento é o **recurso no próprio INSS**. Dois clientes já tinham recebido a promessa errada (04 e 05/09, com o áudio `gravado` junto) — decidido não mexer no que já foi enviado. A troca vale só para maternidade (urbano e rural): texto fixo, tarefa do prompt, vocabulário (`REGRA_VOCABULARIO_RECURSO`) e uma trava que proíbe a IA falar em Justiça, juiz ou processo judicial. BPC, incapacidade, pensão e o resto seguem prometendo a ação, que é o que o áudio gravado promete.

- **O assunto sai de `servico`, não de `benefit_type`.** Nos 409 indeferimentos da base o `servico` reconhece os **64** de maternidade ("SALÁRIO-MATERNIDADE URBANO" 54, "RURAL" 10) e o `benefit_type` reconhece **6** — está vazio em 125 e guarda recorte de e-mail no resto. Pelo mesmo motivo `beneficioLegivel` passou a consultar o serviço quando o benefit_type não diz nada: era isso que fazia a mensagem de maternidade sair como "seu pedido no INSS". Quem não trouxer serviço (277 requerimentos) segue no texto de sempre — decisão de escopo do usuário.
- **O áudio tem chave própria**, `indeferido:maternidade`, servida pelo caminho de sempre (roteiro fixo gerado uma vez por TTS e guardado no catálogo `inss_audio_mensagens`). O gravado da chave `indeferido` promete ação judicial e continua valendo para os outros benefícios; quando a equipe gravar a voz dela para maternidade, é trocar a linha do catálogo por `origem = 'gravado'`. Sem isso o cliente leria recurso e ouviria Justiça no mesmo grupo.
...
- **Busca só por chave exata** — `buscarProcuracaoDoCliente` (`lib/inss-procuracao.ts`): `lead_id` → `outorgante_cpf` = `cpf_segurado` → nome idêntico ao **outorgante** → nome idêntico ao **representante**. Acerta **34 dos 58 (59%)**. Sem chave, devolve `null` de propósito.
- **Acento derrubava a chave de nome** (corrigido 03/09/2026, `padraoSemAcento`): o `nome_segurado` chega do INSS sem acento (938 requerimentos, **1** acentuado) e no ZapSign está com (470 outorgantes, 603 signatários, 133 representantes). Como o `ilike` do Postgres não normaliza acento, o filtro devolvia **zero linhas** e a conferência em JS — que remove acento e teria casado — nunca rodava, porque recebia lista vazia. O comentário antigo (*"a conferência final é em JS"*) descrevia a intenção, não o que acontecia. Agora cada letra acentuável vira `_` no padrão do `ilike`, e quem decide continua sendo a igualdade exata sem acento em JS. Só funciona porque `zapsign_documents` está em **NFC** (conferido) — em NFD o acento ocuparia dois caracteres e o curinga não casaria. Custo medido do bug: na exigência de 03/09 o cliente recebeu o aviso sem o PDF, com o documento no banco, e respondeu no grupo *"Que papel é esse"*.
- **O `lead_id` do documento costuma apontar para o lead errado**: 454 leads têm `ocr_source = 'zapsign_procuracao'` — são criados pelo próprio OCR da procuração, um minuto antes do documento, com o CPF e o endereço extraídos do PDF (`action_source = system`). O atendimento cria outro lead, manual, e é nele que ficam o grupo e o requerimento. Resultado: **632 dos 1.205** documentos com vínculo apontam para esse lead automático, e por isso a chave `lead_id` acerta pouco (2 de 38 na medição de 02/09). Não são leads duplicados por engano — têm funções diferentes; fundi-los seria a correção errada.
- **O nome da criança está DENTRO do PDF** (achado do usuário, 01/09/2026). A procuração de menor é lavrada assim: *"OUTORGANTE: BENTO DA SILVA EMILIANO, BRASILEIRO, SOLTEIRO, MENOR, NESTE ATO REPRESENTADO POR SUA GENITORA GABRIELY DA SILVA"* — o outorgante é a criança (que é o `nome_segurado` do requerimento) e a mãe é a representante. As colunas `has_representative`, `representante_name` e `representante_parentesco` guardam esse par, preenchidas pela edge **`zapsign-extract-pdf`** (existe no Externo e **não está no repo** — ver [migrations-repo-drift]). Por isso a busca casa pelos dois lados: conforme em nome de quem o requerimento foi protocolado.
- **Backfill de 01/09/2026**: 1.273 documentos assinados nunca tinham passado pela extração. Rodados em 85 chamadas de 15, ~12 min, **0 erros**, **801 com representante identificado (63%)**. Efeito medido: cobertura de 41% → **59%**; `pdf_extracted_at` de 1.337 → 2.655; `representante_name` de 131 → **740**; `outorgante_cpf` de 1.296 → 2.313; `endereco_cidade` de 1.284 → 1.946. Sobraram 680 documentos `status = 'pending'`, que a edge não processa — medido, não trariam ganho.
- **O backfill não dispara nada retroativo** (pedido do usuário, 01/09/2026: *"só deve mandar caso seja um email novo"*). Ele só preenche colunas de `zapsign_documents`. Quem decide o envio são as três travas de sempre, em série: `notified = false` (38 das 65 exigências históricas já estão notificadas e não voltam), o corte `INSS_ZAP_CLIENTE_DESDE` (barra 26 das 27 pendentes, por serem anteriores a 26/08) e `jaAvisouEsseTipo`. Na medição do dia, **1** evento pendente passaria — o e-mail de 30/08, que é legítimo.
...
- **A legenda do anexo mora num lugar só** (03/09/2026): `LEGENDA_PROCURACAO` em `lib/inss-zap.ts`, importada pelos três que enviam — estava copiada literal nos três. Tem que ser idêntica porque é **por essa frase que se audita**, no espelho das mensagens, se o PDF chegou ao cliente; cópias que divergem deixam a auditoria cega. O texto virou passo a passo numerado e diz **onde** assinar: a linha em branco no fim do documento, cujo rótulo varia — **OUTORGANTE** quando o titular assina, **REPRESENTANTE GERAL** quando é a mãe pelo filho menor (conferido nos dois PDFs de teste). A versão anterior (*"é só imprimir, assinar à caneta"*) deixava em aberto justamente as duas coisas que o INSS recusa: assinatura fora da linha e imagem de assinatura colada.
- **Primeiro envio comprovado em 03/09/2026.** Até essa data **nenhum PDF havia saído**: a única exigência de procuração desde o deploy de 01/09 caiu num cliente que não tem documento nenhum no ZapSign (0 por `lead_id` e 0 por nome), e a atividade carimbou *"NÃO LOCALIZEI A DESTE CLIENTE"* — o robô não chutou, que é o comportamento desenhado. Três envios de teste ao número do escritório voltaram `Delivered` em `/message/find`, o último saindo **do próprio `inss-procuracao-vincular` em produção**, não de payload montado à mão. O SHA256 do arquivo entregue é igual ao do `original_file_url` no S3: nada é gerado, só localizado.
- **Prova de envio é a UazAPI, não o espelho**: os envios 1:1 entregues **não** geraram linha em `whatsapp_messages` (enquanto 129 documentos outbound 1:1 de outras origens espelharam no mesmo período; causa não apurada). `whatsapp_messages` serve para provar que uma mensagem **saiu**, nunca que **não saiu** — para isso, `/message/find` pelo `messageid`.
...
- Testes: `src/lib/__tests__/inssProcuracao.test.ts` (7 casos com despacho real) e `inssProcuracaoBusca.test.ts` (9 casos da busca, incluindo a proteção contra casar pedaço de nome).
