...
**Nome divergente também para de sumir calado** (04/09/2026). Quando o nome do segurado no requerimento não bate com nenhum nome do lead, o robô se recusa a mandar — a mensagem iria para o grupo de **outro cliente** (`zap_status = 'suspeito'`). A recusa está certa: medindo os 38 eventos parados assim, só **2** eram a mesma pessoa com grafia diferente (DANIELLE/DANIELE, RAINARA/RAYNARA) e **36 eram nomes de fato distintos**. O defeito nunca foi a checagem — foi ela não avisar ninguém: ficava num `console.warn` e a atividade nascia muda. Entre 16/06 e 03/09 isso engoliu **17 indeferimentos e 7 deferimentos**.
...
- **A régua para auditar este fluxo é contar `zap_status` por valor e perguntar, para cada um, quem fica sabendo e onde.** Foi assim que os três desfechos mudos apareceram — `erro` (2), `suspeito` (38) — ao lado de `sem_grupo` (11), que já avisava desde 31/08. Se a resposta for "uma coluna" ou "um log", não fica sabendo ninguém.
