...
- Grupos: busca; "Atualizar dados em lote" (data/criador via UazAPI); "Filtrar e ordenar" (escopo, ordenação, vínculo, funil, período, ocultos); **Modo auditoria** (tabela tipo planilha: nº lead, nº caso, **Processos**, nomes, criado em/por; por linha: abrir conversa, abrir/vincular lead, atualizar dados, editar nº do funil — renomeia o grupo no WhatsApp —, ver contatos, excluir).
  - Coluna **Processos**: quantos processos estão vinculados ao caso daquela linha. Segue a hierarquia Lead → Caso → Processos (skill `lead-vs-case-identity`): resolve o lead do grupo em `legal_cases` e conta `lead_processes` por **`case_id`** — `lead_id` é só espelho de conveniência e não serve de fonte de verdade. Clicar no número abre um painel lateral com a lista (nº CNJ, status, tipo, POP, tribunal); clicar num processo abre a ficha completa (`ProcessDetailSheet`, a mesma da aba Processos).
  - Filtros de auditoria (chips ao lado de "Com lead / Sem lead", e bloco "Divergências" em "Filtrar e ordenar"): **Caso sem processo** (caso existe, nenhum processo cadastrado) e **Grupo em +1 lead** (mesmo grupo vinculado a leads diferentes — a linha ganha ⚠ com a contagem). **Casos sem grupo** abre painel próprio (`CasesWithoutGroupSheet`), porque a lista é de grupos e um caso sem grupo não tem linha nela: parte de `legal_cases`, mostra os órfãos, busca o grupo pelo número do caso e vincula (`lead_whatsapp_groups`), com opção — marcada por padrão — de renomear o grupo no WhatsApp via `regenerate-lead-name`.
  - Sufixo âmbar **+N** na coluna = processos pendurados no lead **sem `case_id`** — inconsistência (processo não existe fora de um caso). Não somam na contagem; aparecem em bloco separado no painel para correção. Em 03/08/2026 eram 94 registros, criados entre 05 e 15/07/2026, sendo 91 em leads que sequer têm caso.
