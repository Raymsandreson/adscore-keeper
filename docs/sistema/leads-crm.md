...
- **Quem da equipe entra no grupo** (15/09/2026) — campo no bloco "Criar grupo do WhatsApp automaticamente". O edge `create-whatsapp-group` monta os participantes só a partir de INSTÂNCIAS (`board_group_instances`, instância do acolhedor, `always_add_to_closed_groups`); quem não tem instância — a maior parte da equipe — não entrava em grupo nenhum, mesmo com telefone cadastrado. O seletor (`src/components/leads/GroupTeamMembersPicker.tsx`) lista os membros com `profiles.phone` preenchido (fonte: Equipe → perfil do membro; quem está sem número aparece desabilitado, com o motivo) e os escolhidos entram logo depois da criação, via `manage-whatsapp-group-participants` (`action: 'add'`, instância-autora como ator — ela é admin do grupo que acabou de criar). Roda antes do resumo automático, então quem entra já encontra a mensagem no grupo. Quem bloqueia ser adicionado não entra: o toast diz quantos de quantos entraram e manda enviar o link de convite — não existe adicionar à força. Nada muda no edge do Externo (que está em v31, atrás do repo).
- **Rodapé do extrator de IA não foge mais para o lado** (15/09/2026) — o `DialogContent` do shadcn é um grid de coluna única: um filho mais largo que a caixa (imagem colada sem `max-w`, valor extraído sem ponto de quebra) estica a coluna inteira e leva junto o rodapé, que é outro item do mesmo grid. Medido: filho de 1000px → `scrollWidth` 1048 contra 655 visíveis, e "Usar Dados Selecionados" 335px fora da tela, só alcançável rolando pro lado. Cabeçalho, stepper e rodapé agora ficam fora da área que rola (`flex flex-col` + `overflow-hidden`, corpo com `flex-1 min-h-0 min-w-0 overflow-y-auto`); a rolagem lateral, quando acontece, fica só dentro do corpo. Padrão já usado em ~40 dialogs do repo.
...
- Avatar do card: mostra a foto do acolhedor quando o lead tem `acolhedor` atribuído (mapa nome→foto em `src/lib/acolhedorPhotos.ts` + `src/assets/acolhedores/`; jul/2026: João Manoel, Analyne, Bruno Dantas, Juliana Pimentel, Luiz Ricardo, Grazielle; set/2026: Renan Mendes, Fabrício Figueiredo). Sem acolhedor mapeado, cai nas iniciais do nome do lead. Acolhedor novo = adicionar foto + entrada no mapa. A mesma foto aparece no título do dialog "Editar Lead" (canto superior esquerdo), refletindo na hora a troca no campo Acolhedor.
- Campo Acolhedor (Editar Lead e Adicionar Lead): combobox com busca por digitação e foto ao lado do nome. No board Trabalhista (Acidente de Trabalho) as opções são só os acolhedores oficiais (lista canônica em `src/lib/trabalhistaAcolhedores.ts` — 8 desde 08/09/2026, com Renan Vieira Mendes e Fabrício Figueiredo); nos demais boards, lista completa de perfis. Acolhedor sem conta no sistema (caso do Fabrício) só existe no board Trabalhista, porque nos outros o seletor lista `profiles`; para entrar nos demais funis precisa de convite pela tela Equipe. Acolhedor novo = entrada nesta lista + foto + linha em `public.acolhedores` no Externo (é ela que alimenta o filtro por pessoa da lista e o avatar curado). Leads antigos com acolhedor fora da lista mantêm o valor até alguém trocar. O dialog "Cadastrar Caso Viável" (Notícias) tem lista restrita própria por user_id, separada desta.
