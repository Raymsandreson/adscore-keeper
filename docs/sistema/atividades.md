...
- **O que a mensagem carrega** (`src/components/activities/buildActivityMessage.ts` — função única, usada pela tela de Atividades e pelo `ActivityFullSheet`):
  ...
  - **Por que antes vinha faltando** (medido em 05/08/2026, 3.346 atividades humanas em 15 dias): o contexto do fluxo saía vazio em ~35% dos casos. 593 (18%) tinham checklist do lead em **outro board** que não o POP escolhido na atividade — o hook filtrava por `lead_id + board_id` e devolvia nada. Agora cai para o funil do **próprio lead** quando o POP pedido não tem checklist. E a fase sumia quando o `stage_id` da instância já não existia no board (fase renomeada/removida: 77% das instâncias do POP-BPC, 65% das do Acidente de Trabalho) — o rótulo é recuperado do próprio id (`captação_e_triagem_1776178507796_awyv` → "Captação e triagem"); ids legados curtos (`done`, `dm`, `whatsapp`) continuam sem linha.
  - Variáveis de template disponíveis em `TEMPLATE_VARIABLES` (`useActivityMessageTemplates.ts`): `{{etapa}}`, `{{objetivo}}`, `{{passo_atual}}`, `{{workflow_info}}`, `{{progresso}}`, `{{progresso_detalhado}}`, `{{tempo_dedicado}}`. `{{progresso}}` era usada pela auto-injeção mas não existia como variável — template que a referenciasse ficava com o bloco vazio.
