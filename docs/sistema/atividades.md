...
- **O que a mensagem carrega** (`src/components/activities/buildActivityMessage.ts` — função única, usada pela tela de Atividades e pelo `ActivityFullSheet`):
  - Ao **cliente**: `*Etapa:*` / `*Objetivo:*` / `*Passo atual:*` + `*📊 Progresso do caso: N% concluído*` + `*⏱️ Tempo dedicado a esta atividade:*`. Injetados automaticamente quando o template salvo não referencia `{{workflow_info}}` / `{{progresso}}` / `{{tempo_dedicado}}`.
