...
- **O que a mensagem carrega** (`src/components/activities/buildActivityMessage.ts` — função única, usada pela tela de Atividades e pelo `ActivityFullSheet`):
  - **Ordem fixa do bloco** (macro → detalhe, igual pro cliente e pro assessor): processo → barra de progresso → `*Etapa:*` / `*Objetivo:*` / `*Passo atual:*` → tempo dedicado. As auto-injeções ancoram por padrão (regex), não por string fixa, porque o texto do processo e o rótulo do progresso variam.
  - **Nº do processo**: sai `*Processo n° <nº>* — <título>` lido do dado vivo (`lead_processes`). Quando o processo **não tem número cadastrado** — 901 de 1.728, 52% da base em 05/08/2026 — a linha vira `Referente ao processo "<título>"`; antes saía `n° "—"`, que não informa nada. A atv com processo e **sem caso** (105 em 90 dias) agora carrega o processo pelo próprio `process_id`; antes `caseProcesses` ficava vazio e a mensagem caía no `process_title` congelado, que em 47% das atividades com processo (3.519 de 7.554) não tem número.
  - **Barra de progresso** (`progressBar()`): 10 blocos `🟩`/`⬜` + a %. Emoji em vez de `█`/`░` porque o WhatsApp não garante fonte monoespaçada — blocos unicode saem desalinhados ou sem glifo em parte dos Androids. Fluxo iniciado nunca mostra barra vazia (arredonda pra 1 bloco a partir de 1%).
  - **Rótulo do progresso segue a origem do fluxo**: `Progresso do processo` / `Progresso do POP` / `Progresso do atendimento` (o funil não vira "lead" na mensagem — jargão interno não vai pro cliente).
  - Ao **cliente**: o bloco acima com a barra + `*⏱️ Tempo dedicado a esta atividade:*`. Injetados automaticamente quando o template salvo não referencia `{{workflow_info}}` / `{{progresso}}` / `{{tempo_dedicado}}`.
  - Ao **assessor**: o mesmo, mais a quebra do progresso (fases, objetivos da fase atual, passos do objetivo atual) e a autoria.
  - **Tempo dedicado voltou em ago/2026** — a decisão de jul/2026 tinha tirado de todas as mensagens. Soma `active_seconds` de todas as sessões da atv (`activity_time_entries`, hook `useActivityTimeTotal`, índice `idx_ate_activity`) e conta a sessão em curso ao vivo. Abaixo de 1 minuto a linha não sai.
  - **De qual fluxo vem o progresso** (regra do usuário, ago/2026 — cadeia ordenada em `useActivityStepContext`, vence o primeiro candidato que tiver checklist do lead):
    1. **POP do processo** vinculado à atv (`lead_processes.workflow_id`);
    2. **POP próprio da atv** (`lead_activities.workflow_id`) quando não há processo — aí a atv é a *referência* do acompanhamento e o nº dela (8 primeiros hex do UUID, mesmo código de `/atv/:code`) sai na mensagem como `*🔖 Atividade de referência do POP:*`;
    3. **Funil de vendas do lead** (`leads.board_id`), incluído como último recurso mesmo quando quem chamou não o passou (o `leadPreview` pode não ter carregado).
    Antes a ordem era POP da atv > POP do processo > funil, sem cadeia: se o board escolhido não tivesse checklist, o contexto vinha vazio e **nada** de fluxo saía. A troca de ordem afeta 76 atividades em 30 dias (1,1%) — as que têm POP próprio divergente do POP do processo. `lead_checklist_instances.process_id` é 100% NULL (0 de 25.667), então "POP do processo" significa as instâncias do lead no board daquele POP, não instâncias por processo.
  - **Por que antes vinha faltando** (medido em 05/08/2026, 3.346 atividades humanas em 15 dias): o contexto do fluxo saía vazio em ~35% dos casos — 593 (18%) tinham checklist do lead só em outro board, 572 (17%) não tinham checklist nenhum. E a fase sumia quando o `stage_id` da instância já não existia no board (fase renomeada/removida: 77% das instâncias do POP-BPC, 65% das do Acidente de Trabalho) — o rótulo é recuperado do próprio id (`captação_e_triagem_1776178507796_awyv` → "Captação e triagem"); ids legados curtos (`done`, `dm`, `whatsapp`) continuam sem linha.
  - Variáveis de template disponíveis em `TEMPLATE_VARIABLES` (`useActivityMessageTemplates.ts`): `{{etapa}}`, `{{objetivo}}`, `{{passo_atual}}`, `{{workflow_info}}`, `{{progresso}}`, `{{progresso_detalhado}}`, `{{tempo_dedicado}}`, `{{atv_referencia}}`, `{{atv_numero}}`. `{{progresso}}` era usada pela auto-injeção mas não existia como variável — template que a referenciasse ficava com o bloco vazio.
  - Cobertura: `src/components/activities/__tests__/buildActivityMessage.test.ts`.
