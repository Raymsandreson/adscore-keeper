...
### Seleção em lote (adiar, passar para outro assessor ou excluir)
Botão **"Selecionar"** acima da lista liga o modo de seleção: cada cartão ganha caixa de marcação, o clique passa a marcar em vez de abrir a ficha (shift+clique marca o intervalo) e as ações individuais do cartão somem. Com algo marcado, o rodapé da coluna mostra a contagem e três botões: **"Excluir"**, **"Adiar N"** e **"Passar para…"** — este último abre um painel lateral com a busca de assessor (`BulkReassignSheet.tsx`).
...
- **Adiar em lote** (02/09/2026): o mesmo popover do "Adiar" da ficha (`PostponeActivityPopover`, com os atalhos de `postponeDates.ts` — próximo dia útil, 3 dias úteis, próxima segunda, 1 semana, ou data manual a partir de hoje), agora sobre as marcadas. Troca **só o prazo** — grava `deadline` **e** `notification_date` no dia escolhido (igual ao individual: quem adia quer a atividade inteira no dia novo, não o aviso num dia e a execução em outro), não conclui nada, não cria filha e **não mexe no responsável**. Antes disso, adiar uma fila inteira significava abrir uma ficha por vez; o atalho que a equipe tentava — "Passar para…" escolhendo o **próprio nome** com data nova — não funciona e nunca vai funcionar, porque `montarPlano` só reescreve quem muda de dono (o painel agora diz isso na linha "já são de X", em vez de aplicar zero em silêncio).
  - **Duas exceções saem do lote e são relatadas no toast**, em vez de derrubar tudo (`planejarAdiamento`, função pura testada em `src/lib/bulkPostpone.ts`): **concluída** não se adia (a data dela registra quando o trabalho foi feito) e prazo que cai em **ausência registrada** de qualquer responsável — titular ou co-assessor — é pulado, que é a mesma trava que o `updateActivity` aplica no adiar individual, só que aqui pessoa a pessoa. `member_time_off.user_id` é **Cloud UUID** (conferido em 02/09/2026: 4/4 dos donos de ausência casam por `cloud_uuid` no `auth_uuid_mapping`, 3 deles com `ext_uuid` diferente), então os responsáveis passam pelo `remapToCloudSync` antes da checagem.
  - **Autoria e aviso**: carimba `updated_by` (senão vira "atualizada por —"), grava audit log `adiamento_atividades` com os ids, o novo prazo e o que foi pulado, e avisa **um por pessoa** quem não é você (`activity_notifications`, tipo `rescheduled` — o que o listener já rotula "🗓️ Atividade reagendada"). O update usa `.select('id')` e confere a contagem: update que não casa linha nenhuma (RLS + sessão anônima expirada) volta **sem erro**, e sem isso a tela diria "adiadas" com nada gravado.
  - Se a ficha aberta no painel estiver no lote, o formulário é sincronizado com o prazo novo — senão um "Salvar" logo depois regravaria a data antiga por cima do adiamento.
...
**Seleção em lote**: as linhas com atividade ganham caixa de marcação e as mesmas três ações da lista (Excluir / Adiar / Passar para…). Como a agenda alcança atividade que **não** está no lote carregado pelos filtros da página, o lote daqui trabalha por id — `handleBulkDelete` aceita ids explícitos, e o "Adiar" e o "Passar para…" buscam os objetos completos por id antes de agir (o adiar precisa deles para saber o que é concluída e quem está de férias); sem isso o lote sairia parcial e em silêncio.
...
- **Notificação (🔔) tem data e hora** (25/08/2026) — uma caixa `datetime-local` ao lado do Prazo. A data continua em `lead_activities.notification_date` (**DATE**, quem alimenta agenda, Google Calendar, BulkReassign e a contagem de carga do dia) e a hora vai para a coluna nova `notification_at` (**timestamptz**, migration `20260825120000` no Externo). Coluna nova em vez de `ALTER TYPE`: a `notification_date` é lida como string `yyyy-MM-dd` em três caminhos (o `.eq()` do `ConfirmDialogDateFields`, o regex do autoApply e o `slice(0,10)` do `diaDaAtividade`), e trocar o tipo reescreveria as 39.523 linhas mexendo em todos de uma vez. Helpers e testes em `src/lib/notificationDateTime.ts`.
  - **Meia-noite é "sem hora"**. As 22.247 atividades com data de aviso anteriores à coluna não têm hora nenhuma para exibir, e "às 00:00" numa mensagem ao cliente seria pior que não dizer nada. Elas abrem com a hora vazia. A mensagem ao **cliente** diz sempre *"até o final do dia"*, com ou sem hora marcada (14/09/2026 — ver "A saudação cumprimenta todos os clientes" abaixo); a hora continua na agenda e na linha *"Notificação:"* da mensagem ao **assessor**.
...
- Envio ao grupo: "Copiar" (mensagem pronta), "Avaliação" (gera link público 0–5⭐), "Enviar ao Grupo / Enviar ao Assessor" (preview editável, escolha de instância, opção "Incluir gravação da ligação").
  ...
  - **A saudação cumprimenta todos os clientes, e sem hora marcada** (14/09/2026). Duas coisas na mesma mensagem, vistas no processo `0100419-74.2021.5.01.0281` (Ação Trabalhista, **7 autores**, todos nossos clientes). (1) A saudação saía *"Sr(a). Changrillayne Biazini"* — a **primeira** parte da lista e só ela, porque o `rawClientCandidate` pegava `processClientNames[0]`. Os outros 6 autores leem a mesma mensagem no grupo e não eram cumprimentados. Agora vai o **primeiro nome de cada parte** do polo que representamos: *"Sr(a). Changrillayne, Deolinda, Edson, Elisangela, Maria, Wandrey e Wendey"*, **sem teto** de quantidade (decisão do usuário) — processo com 30 autores sai com os 30. O polo continua vindo de `cliente_polo` ou da detecção por OAB (`detectClientPolo`), então a lista é sempre do **nosso** lado, nunca do adversário; advogados ficam de fora como antes; parte que a API devolve em **dois papéis** (o perito deste processo) não aparece duas vezes; **pessoa jurídica** entra com a razão social inteira, porque *"Sr(a). Ampla"* não é o nome de ninguém (`isPessoaJuridica`: `tipo_pessoa=JURIDICA` ou CNPJ preenchido). Nome digitado à mão no formulário continua vencendo tudo, e sem partes estruturadas ainda sobra o título do polo (*"Edson Ribeiro e outros"*), que já vem pronto e não dá para quebrar em primeiros nomes. Funções novas em `buildActivityMessage.ts`: `extractPartyFirstName` (o `extractClientFirstName` limpa o rótulo mas devolve o nome **inteiro** — o nome dele mente desde antes) e `isPessoaJuridica`. (2) A frase de retorno prometia **horário** — *"no dia 24/09/2026 quinta-feira, às 09:00."* — sempre que a atividade tinha hora. Agora é sempre *"até o final do dia"*, e `{{data_retorno}}` também perdeu a hora: o template salvo do board **Acidente de Trabalho** emenda *", até o final do dia"* logo depois dela e sairia duplicado. Travado por `buildActivityMessage.saudacao-partes.test.ts`, montado com os envolvidos reais do processo.
  ...
  - **A gravação da ligação passa a ir junto com o texto** (14/09/2026). A caixa "Incluir gravação da ligação" existia, mas quase nunca chegava a aparecer: a busca do anexo vivia num `useEffect [activityId]`, ou seja, rodava quando a atividade **abre** — e a pessoa grava a ligação **depois** disso (o `ActivityCallRecorder` só cria o anexo no fim da gravação). Com `recordingUrl` nulo, a caixa não era renderizada e o `sendToGroupNow` nunca chamava `sendRecording`: o texto saía sozinho, sem aviso nenhum de que o áudio ficou para trás. Não era caso de borda, e isso foi medido em 14/09/2026 por **dois caminhos independentes**, ambos no espelho do webhook no Externo — que é onde o envio deixa rastro legível, já que o registro outbound vive no Cloud. Pelo **deep link**, casando mensagem→atividade pelo `openActivity=<uuid>` que o texto carregava: das **89** atividades com gravação que notificaram em 30 dias (79 delas em grupo), **89** gravaram antes do envio e **73** a menos de 30 min. Pelo **título**, casando `*Assunto da atividade:*` com o lead dono do grupo: das **243** atividades, **232** (95%) gravaram antes, 223 a menos de 30 min — e as 11 exceções gravaram de 4 a 21 dias depois, atividade reaproveitada, não contraexemplo. A população é confiável dos dois lados porque `attachment_type='audio'` é exclusivamente gravação de ligação: 923 anexos em 30 dias, todos `Gravação da ligação.webm`. O caminho normal era justamente o que não funcionava. Duas ressalvas para quem reconferir: a janela é móvel (`now() - interval '30 days'`), então o número anda sozinho — 87 de manhã, 89 à tarde do mesmo dia; e a âncora do deep link **está secando**, porque o link saiu da mensagem ao cliente em 25/08/2026 (71 das 89 são anteriores a essa data, e mensagem de cliente com link caiu de ~4.500/mês em jul-ago para 53 em setembro) — em algumas semanas ela só enxerga mensagem ao assessor, e quem quiser refazer a conta terá de usar o casamento por título. Hoje a leitura acontece no **clique** de "Enviar" (`fetchLatestRecording` dentro de `openPreview`), com três cuidados: `ensureExternalSession()` antes da query, porque `activity_attachments` tem policy `TO authenticated` e, sem sessão, o PostgREST devolve `200 []` — o falso-vazio silencioso do incidente de 17/08/2026; erro de leitura vira **toast + log** em vez de virar "não tem áudio"; e a caixa **nasce marcada** (gravou e escreveu na mesma atividade, o padrão é os dois irem juntos) mostrando **nome e hora do arquivo**, que é o que impede mandar uma gravação antiga sem perceber. Trocar de atividade limpa o estado — foi assim que um áudio saiu no grupo do lead anterior em 06/08/2026. Travado por `SendToGroupSection.gravacao.test.tsx`, que verifica o ponto certo: a leitura no clique, não na montagem.
  - **Na ficha em aba lateral a gravação não tinha para onde ir** (14/09/2026). A `ActivityFullSheet` — a ficha aberta pelo caso, pelo kanban ou pelo chat — gravava a ligação e parava aí: nenhum botão mandava aquele áudio a lugar nenhum. Ganhou **"Enviar áudio"** ao lado do "Enviar", com o destino resolvido do **banco** por `resolveLeadAudioTarget(leadId)` e nunca do `leadPreview` da tela (foi exatamente esse state não resetado que mandou áudio para o cliente errado em 06/08/2026). Sem grupo nem telefone o botão não aparece; o estado zera ao trocar de atividade e ao fechar a ficha.
- **Aba "Histórico": quem criou e por quantas mãos a atividade passou** (08/09/2026, `ActivityMovementsPanel` + `src/lib/activityHistory.ts`). A aba só mostrava a **sequência** do "Concluir + próxima"; a vida da atividade em si — repasses, situação, edições — existia no banco e **nenhuma tela lia**: o `lead_activity_audit_log` tinha 44.205 eventos (desde 18/07/2026) e era só escrito pelo trigger `log_activity_audit`. Agora a aba abre com a linha do tempo: *"Passou de Luana Barros para Gisele Borges dos Santos · 01/09/2026 às 10:50 · por Keliane · Pendente"*, com a situação vigente em cada repasse, e a sequência de continuidade logo abaixo. Medido em 08/09/2026: **1.029 repasses** registrados, e a atividade campeã (07252a98, CASO/PREV de agendamento de perícia) trocou de responsável **5 vezes** em 10 dias.
  - **Autoria deixou de ser "—"**: o rodapé imprimia traço para **10.152 atividades vivas** (25,5%) sem `created_by`, sendo que **todas** dizem a fonte em `action_source` — 5.332 são de robô com o nome dentro de `action_source_detail` ("Etiqueta do WhatsApp" 4.219, "Robô do INSS" 624, "Follow-up automático" 330, "Onboarding automático" 62, "Robô do ZapSign" 57). Hoje a ficha mostra o robô (com o símbolo do `RobotBadge`); quando é `manual` sem autor (4.798, quase todas de maio/2026) ela diz **"autor não registrado"** em itálico, em vez de fingir que foi o sistema.
  - **Os repasses também aparecem no rodapé** (`ActivityHandoffSummary`), embaixo de "Criado por" e "Última atualização por": *"Repassada 2 vezes: Luana Barros → Gisele Borges · 01/09/2026 às 10:50 · por Keliane · Pendente"*. O histórico completo mora numa aba, e quem lê o rodapé já está com a pergunta seguinte na cabeça ("e de quem era antes?") — trocar de aba para responder isso era um clique a mais no lugar errado. Sem repasse gravado o bloco **não desenha nada**: linha dizendo "nenhum repasse" seria ruído em toda atividade.
  - **Rotina de servidor ≠ sessão sem login.** Evento com `actor_id` nulo é rotina rodando sem `auth.uid()` ("por rotina do sistema"); `actor_id` que não resolve nome é a sessão anônima do Externo ("por sem identificação") — a distinção existe porque o trigger chama as duas de `actor_kind='system'` (ver `identidade-de-usuario.md`). Nomes resolvem por `profiles.id` **e** `user_id` do Externo, que divergem para parte da equipe.
  - **A tela não finge cobertura que não tem**: atividade anterior a 18/07/2026 ganha aviso de que nada foi gravado antes disso, e atividade anterior a 21/08/2026 sem repasse avisa que a **troca de responsável** só passou a ser registrada naquela data (migration `20260821120000_audit_registra_troca_de_responsavel`). "Sem registro" não é "não houve".
  - Leitura pura, uma query por atividade no índice `idx_laal_activity` (máximo real: 17 eventos numa atividade; média 2,2). A tabela ainda não está no `types.ts` gerado — daí o cast `db as unknown as SupabaseClient` no painel. Coberto por `activityHistory.test.ts` (18 casos) e `ActivityMovementsPanel.test.tsx` (4 casos, com as linhas reais do audit).
...
**Os passos saíram de dentro da atividade: agora é aba lateral** (14/09/2026) — o painel de passos era um `<Collapsible>` que abria DENTRO do cabeçalho da atividade, numa caixa de 320px, e mostrava **uma fase por vez** (navegação "Fase anterior / Próxima fase"). A equipe parou de usar: ficha e POP disputando a mesma tela, e um marco por "aba" obrigava a caçar onde o processo parou. O que existe agora, no `LeadFunnelProgressBar`:

- **A barra continua na atividade** (é a medida, tem que estar à vista). Clicar num segmento ou no percentual abre um `Sheet` com `side="left"` — ao LADO da ficha, que ocupa a direita, nunca por cima dela (skills `ui-sem-redirecionar` + `ui-sem-sobreposicao`). Fechar devolve a pessoa onde estava.
- **Todas as fases numa lista só**, empilhadas (`instancesPorFase`), sem navegação por fase. Clique num segmento da barra abre a aba **rolada até aquela fase**; clique no percentual abre **no passo atual**.
- **Passo atual** = 1º passo vivo não-marcado, varrendo a partir da fase em que o processo está e dando a volta nas anteriores. Ao abrir, a aba rola até ele (e abre à força a fase + objetivo dele, senão a rolagem cairia numa seção recolhida). Ele fica com anel e selo "passo atual"; há o atalho "Ir ao passo atual" no cabeçalho. A rolagem NÃO se repete a cada passo marcado — o alvo entra por `ref` de propósito, senão a tela pularia debaixo do dedo de quem marca vários seguidos.
- **Três níveis de recolhimento** (`nivelVisao`): **Marcos** (só as fases — a fase recolhida ainda mostra os marcos já atingidos, senão recolher viraria esconder), **Objetivos** (fases + objetivos, sem passos) e **Passos** (tudo). Clicar numa fase/objetivo vale como exceção ao nível; trocar de nível zera as exceções.
- **Selo "hoje"** no passo marcado no dia (casa por rótulo normalizado com a RPC `pop_steps_log`; passo-pergunta entra no log como `rótulo — resposta`, daí o `startsWith`).
- O que **não** mudou: marcação, log (`log_checklist_step`), cascata de sub-itens, passo-pergunta, "marcar todos", `PopCatchUpSheet` e o cálculo de percentual. Testes: `LeadFunnelProgressBar.aba-lateral.test.ts` (desenho) e `.aba-lateral.render.test.tsx` (monta e abre de verdade).

**Atividade criada com POP pergunta se algum passo já foi dado** (14/09/2026) — parte do trabalho costuma já ter sido feita antes de a atividade nascer (ligação atendida, protocolo enviado, documento anexado); sem a pergunta o POP ficava parado numa fase que o processo já passou, e o percentual da carteira mentia junto. Como funciona:

- Intent global `abrirPassosDoPop()` (`src/lib/popPassosIntent.ts`), no mesmo padrão do `movimentacaoSheet`/`whatsappChatSheet`. Quem escuta é o `PopPassosAoCriarHost`, montado uma vez no `App.tsx`.
- O host abre a **mesma** aba lateral dos passos (`LeadFunnelProgressBar` com `modo="somente-passos"`) — mesma marcação, mesmo log, mesmas regras. Nada de formulário reduzido paralelo.
- Dois pontos de disparo, os dois humanos: `ActivitiesPage` (a pergunta entra no `ActivityCreatedDialog`, que nesse caso **não** se fecha sozinho em 5s — fechar sozinho seria o mesmo que não ter perguntado) e `ActivityFullSheet` (fecha ao criar, então dispara o intent com `perguntar: true` e o host é que pergunta).
- O POP sai da **mesma cascata** da barra: POP próprio da atividade → POP do processo → funil/POP do lead. Atividade **interna/gerencial** (`is_system`/`is_management`) fica de fora: não há passo de POP a cobrar. Teste: `ActivityCreatedDialog.pergunta-pop.test.tsx`.
