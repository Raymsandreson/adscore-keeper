...
**Véspera e período.** O padrão é véspera: mostra D+1. Na sexta a janela vai até a **segunda**, inclusive sábado e domingo — a segunda também precisa de véspera, e prazo cai em fim de semana (audiência não). Feriado não é considerado: não existe tabela de feriado forense aqui. O menu de data oferece ainda **Próximos 7/15/30 dias** e um **período livre** (`diasDoIntervalo`, teto de 92 dias); em período com mais de um dia a tabela ganha cabeçalho por dia.
...
- **Retorno agendado (↩️)** — data **e hora** de voltar a falar com o cliente/parte, em `lead_activities.callback_at` (timestamptz, migration `20260814190000`). Campo opcional, logo abaixo de Prazo/Notificação e fora da grade das duas de propósito (elas são obrigatórias, este não), com botão de limpar porque `datetime-local` não zera sozinho em todo navegador. Serve para o caso em que a atividade não vence hoje mas alguém precisa ser retomado numa hora marcada — coisa que Prazo (DATE, sem hora) não expressa.
  - **Onde muda alguma coisa**: na lista de pendentes do "Trocar atividade" (`SwitchActivityDialog`), onde a ordenação é `meeting_at > callback_at > deadline > notification_date`. Não aparece em cartão nem na lista principal — `meeting_at` também não, é paridade e não esquecimento.
  - **"Concluir + próxima" leva o valor para a filha**, junto de prazo e notificação: as datas do formulário nesse fluxo são as da *próxima* etapa, e a mãe é concluída sem salvar o formulário. Sem isso o que foi digitado sumiria sem erro.
  - Nasceu **quebrado pela metade** entre jul/2026 e 17/08/2026: o front lia, hidratava, resetava e mandava `callback_at`, mas **nenhum input existia** (a prop `formCallbackAt` chegava ao `ActivityFormCompact` e nada a usava) e **a coluna nem estava criada** no Externo — `select` pedindo por ela derrubava a query inteira no PostgREST (`42703`) e a lista de pendentes aparecia **vazia em silêncio**. Por isso 100% das atividades anteriores estão com `callback_at` nulo. Coluna criada em 14/08 (`70de00332`), input em 17/08 (`f125cd1b5`).
...
**Chips do topo são clicáveis** (desde 18/08/2026): ⏰ atrasadas · 🔁 reagendadas · 📥 a avaliar · ✅ satisfeito · ⚠️ incompleto · ❌ insatisfeito. Clicar num chip abre a **relação daquelas atividades** (mesmos cartões do funil, com cobrança ❗/🚨 nas atrasadas e o formulário de avaliação nos retornos); o chip em foco fica com anel, e "Voltar" devolve à visão anterior. Antes eram números decorativos — dava para ver "40 atrasadas" sem ter como listá-las.

**Três visões**: "Funil" (kanban das 6 colunas), "Calendário" (padrão, por data de prazo/reagendamento) e **"Por assessor"** (desde 18/08/2026) — tabela com a **quantidade de cada status por assessor responsável**, ordenada por quem tem mais atrasadas, depois maior total, depois nome. Cada número é clicável: leva à relação daquele status **daquele assessor** (fixa o filtro de assessor). Rodapé com o total geral. A tabela respeita o filtro de período e ignora o de assessor por construção (ela **é** a quebra por assessor).

A contagem vive em `src/lib/feedbackFunnelStats.ts` (`contarPorAssessor`, `totalGeral`) — mesma classificação das colunas do funil (reagendada pelo status; sem `feedback_outcome` = a avaliar; sem responsável cai em "—"). Testes: `src/lib/__tests__/feedbackFunnelStats.test.ts` e `src/components/activities/__tests__/FeedbackFunnel.chips.test.tsx`.
