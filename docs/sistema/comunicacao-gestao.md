...
- **Quem cuida do caso vem primeiro no `@`**: acima da equipe aparece o bloco "Quem cuida deste caso" com o **responsável processual** e o **acolhedor**, cada um com o selo do papel ao lado do nome — dá pra acionar a pessoa certa sem abrir a ficha do processo. Vale em todo chat interno com caso por trás: ficha do lead, do processo, do caso, da atividade e a conversa/grupo do WhatsApp (o grupo é ligado ao lead por `lead_whatsapp_groups`). Fonte: `leads.processual_responsible_id` e `leads.acolhedor` — processo/caso/atividade herdam do lead. Quando o acolhedor está gravado como apelido genérico ("Atendimento Previdenciário") ou não tem usuário no sistema, o nome aparece rotulado mas sem virar menção.
