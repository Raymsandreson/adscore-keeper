...
- **O formulário vem preenchido pela IA** (desde 06/08/2026): criar atividade a partir de mensagem usa `chat-to-activity` (Railway) e abre o **formulário completo** (`ActivityFullSheet`) — o mesmo caminho do chat interno da equipe. Antes abria o formulário reduzido do WhatsApp com o texto no campo de ditado (`parse-activity-dictation`), e só o assunto vinha preenchido.
  ...
  - **Nº de processo citado no material vira vínculo — e nº torto vira pergunta** (desde 09/09/2026): o número lido casa com `lead_processes.process_number` e traz processo, caso, lead e POP prontos. Quando vem **com um dígito a mais ou a menos** (traço no lugar do ponto, zero sobrando na unidade de origem — `0000846-69-2025-5-08-00009` tem 21 dígitos), o **dígito verificador do CNJ** (módulo 97) reconstrói o número: entre os candidatos possíveis de 20 dígitos, só passa quem fecha o DV. Achado o processo, a ficha abre **sem** o vínculo e mostra uma faixa âmbar no topo — "O material cita o nº X — não existe processo com esse número. O que existe é Y — {processo} · {caso} · {lead}" — com **"É esse — vincular"** e **"Não é esse"**. Vincular por número consertado sem passar pelo assessor seria dar como certo o que ninguém conferiu, e depois ninguém teria como desconfiar.
    - Nenhum candidato fecha o DV → **não sugere nada**. Número aleatório, NB do INSS e protocolo administrativo caem aqui. O reparo nunca cria vínculo sozinho, e o número escrito no material continua aparecendo na tela do lado do achado.
    - **Por que passa longe de detalhe**: sem processo vinculado, o fluxo da atividade cai no funil do lead (`stepBoardId = formWorkflowId || processo.workflow_id || lead.board_id`) e a mensagem ao cliente sai com a etapa do funil — "Etapa: Pós Visita Parceiro · 0% concluído" — no lugar do POP do processo ("Audiência de instrução · 21%"). Foi exatamente o que aconteceu no CASO 307 em 09/09/2026: o número torto derrubou o casamento por número, o vínculo caiu no nome do lead ("Engbel"), e o processo — que existia, no mesmo lead — ficou de fora.
    - `src/lib/cnj.ts` (`digitoVerificadorCnj`, `cnjDvValido`, `candidatosCnj`), `src/lib/vinculoDaAtividade.ts` (`porNumero` → `precisaConfirmar`), faixa e handler no `ActivityFullSheet`. Testes: `src/lib/__tests__/cnj.test.ts`, `src/lib/__tests__/vinculoDaAtividade.test.ts`, `src/components/activities/__tests__/ActivityFullSheet.numero-a-confirmar.test.tsx`.
...
Clica no **relógio ao lado do botão de enviar** (ou "Agendar mensagem" no menu do clipe) — **com o campo vazio, sem precisar digitar nada antes** (12/09/2026). Escolhe dia e hora — com atalhos "Daqui a 1 hora", "Amanhã 8h", "Segunda 8h" — e, se quiser, a repetição: **todo dia, toda semana** (com os dias marcados: seg/qui, por exemplo), **todo mês** ou **personalizado** ("a cada 3 semanas"). A recorrência aceita fim por data (`repetir_ate`) ou por número de envios (`max_envios`); sem nenhum dos dois, repete sem fim. A janela mostra a frase pronta ("Toda segunda e quinta às 08:00") e os **próximos 4 envios** antes de salvar.
...
- **Agendar não exige escrever antes** (12/09/2026). O relógio aparece sempre no modo mensagem, e a mensagem é escrita **dentro da janela** (campo "Mensagem"). Antes o botão só nascia depois de o campo do chat ter texto, e a janela apenas exibia o que já estava digitado — quem ia agendar tinha de escrever primeiro para descobrir que dava para agendar. A montagem do texto que vai para o banco (assinatura `*Nome:*` e `@marcados`) continua num lugar só: o chat passa a função `montarEnvio` para a janela, em vez de um texto já pronto, então agendado e enviado na hora não podem divergir.
- **A data vem da própria conversa** (12/09/2026). `lerQuandoDaConversa` (`src/lib/quandoDaConversa.ts`) lê as últimas falas do interlocutor e traduz "me manda na segunda", "amanhã", "semana que vem", "daqui a 3 dias", "dia 15", "15/09", "às 14h30", "3 da tarde" em dia e hora — a janela já abre marcada nela e **mostra o trecho que a gerou**, para conferência. Sem hora dita, usa 8h. Determinístico de propósito: é aritmética de calendário, e um modelo aqui custaria token, latência e variação sem acertar mais. Nada reconhecido = sugestão padrão de sempre (daqui a 1 hora).
- **A IA escreve a mensagem para a data escolhida** (12/09/2026, botão "Sugerir com IA" dentro da janela). Reusa o motor da sugestão do chat (`gerarSugestaoDeResposta`, mesmo tom, relacionamento e andamento do processo) com uma instrução a mais: o texto **não sai agora**, sai no dia marcado — retoma o combinado em vez de responder no presente. Sem isso a sugestão chegava dias depois falando "acabei de ver".
...
- **Ligar por voz de dentro do chat da ficha (desde 04/09/2026)**: o telefone na barra do rodapé abre a lista "Ligar por voz" — quem cuida do caso primeiro (com o papel ao lado), depois quem já escreveu naquele chat, depois o resto da equipe — e cada bolha ganhou "Ligar" para quem escreveu. Vale em lead, caso, processo, contato, atividade, POP e no painel "Equipe" da conversa do WhatsApp, porque é o mesmo `TeamChatPanel`. A chamada é a que já existia no Chat da Equipe (`CallContext`, WebRTC): abre no card do canto **por cima** da ficha, grava, e ao desligar o `CallSummaryDialog` oferece transcrição e atividade. No telão `/tv/atividades` — que abre o `ProcessDetailSheet` fora do `CallProvider` — o botão simplesmente não aparece (`useCallOptional`). A lista tem **busca por texto** (foco automático ao abrir, `Enter` liga para o primeiro, `Esc` fecha) e quem liga **ouve o "tuu… tuu…"** enquanto chama (1s de tom em 425 Hz a cada 5s, `RINGBACK` em `src/lib/webrtcCall.ts`) — antes o silêncio até o outro atender parecia travamento.
- Código: `src/components/chat/TeamChatPanel.tsx` e `TeamDirectChatPanel.tsx`; componentes de IA em `src/components/ui/AITextActions.tsx` e `AISuggestReply.tsx`; citação em `src/lib/teamChatQuoteEvents.ts` (`formatQuotedMessages`); voz em `src/contexts/CallContext.tsx`.
