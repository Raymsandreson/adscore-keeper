...
- **A foto fica salva entre sessões**: `localStorage` (chave `wa_avatars_v2`, TTL 6 dias) em vez de `sessionStorage`, e o arquivo sobe ao bucket com `cacheControl` de 7 dias — antes o Storage respondia **sem `Cache-Control`** e o navegador rebaixava a mesma imagem a cada abertura.
