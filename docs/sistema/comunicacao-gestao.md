...
- Duas visões: **Lista** (agrupada por urgência — Vencidas, Hoje, Amanhã, Próximos 7 dias, Mais para frente, Sem data) e **Calendário** (mês, com a contagem por dia; dia com pendência vencida fica vermelho, clicar abre as do dia).
...
- Por item: **Feito** (pergunta quem resolveu, pré-selecionando o responsável do caso — fora da conversa não dá para saber quem falou por último), **Abrir conversa** (abre a conversa no **painel de baixo**, sem sair da caixa — a lista sai da frente e volta sozinha ao fechar; empilhar o Drawer sobre o Sheet deixaria dois modais disputando foco e trava de rolagem. Dentro dele, "abrir no WhatsApp" leva à inbox completa quando for preciso o resto das ferramentas) e **Não era** nas detectadas pela IA.
