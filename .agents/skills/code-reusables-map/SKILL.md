...
- `src/lib/vinculoDaAtividade.ts` → `acharVinculoDaAtividade` (nº CNJ → partes → lead) devolve processo + caso + lead + POP pro rascunho. **Não reimplemente busca de processo por número em tela nova.** Número torto (21/19 dígitos) volta com `precisaConfirmar: true` + `numeroLido`/`numeroAchado` — quem chama tem que **perguntar**, nunca preencher o vínculo sozinho.
- `src/lib/cnj.ts` → `parseCnj` (ramo/tribunal/UF/unidade), `formatCnj`, `onlyDigits`, `cnjVariantes` (as formas em que o mesmo nº pode estar gravado, pra `.in()`) e, desde 09/09/2026, `digitoVerificadorCnj`/`cnjDvValido`/`candidatosCnj` (reparo de número com dígito sobrando ou faltando, validado pelo módulo 97). **Qualquer regex de CNJ nova deve usar estas** — a regex `d{7}-?d{2}...` está copiada em 11 arquivos (`grep -rl 'd{7}' src railway-server/src`) e nenhuma delas aceita o número torto que chega do WhatsApp.
