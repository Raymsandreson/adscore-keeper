...
| Passos do POP (de qualquer tela) | `LeadFunnelProgressBar` — na atividade/processo pela própria barra; de fora, pelo intent `abrirPassosDoPop()` de `src/lib/popPassosIntent.ts` | Aba lateral `side="left"` (a ficha ocupa a direita). Todas as fases numa lista só, rolando até o passo atual; níveis Marcos/Objetivos/Passos. Quem escuta o intent é o `PopPassosAoCriarHost`, montado no `App.tsx`. Não faça painel de passos paralelo. |
